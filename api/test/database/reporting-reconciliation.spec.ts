 ...
 * ```
 *
 * **Written before the queries it checks**, because it is the only oracle for a bucket
...
describe('section 20 reconciliation, DCC monthly (Stage 5 Done-when)', () => {
  ...
  const MONTH = '2027-10';
  const OCT_3 = '2027-10-03';
  const OCT_10 = '2027-10-10';
  const OCT_17 = '2027-10-17';
  const OCT_24 = '2027-10-24';
  const SEP_26 = '2027-09-26';

  /** The instant a corrected record changes hands. Fixed, and written to both ends. */
  const CORRECTED_AT = new Date('2027-10-11T09:00:00+08:00');
  ...
  it('both views sum to the same unique-people total', async () => {
    ...
    const sep = await event(SEP_26);
    const e1 = await event(OCT_3);
    const e2 = await event(OCT_10);
    const e3 = await event(OCT_17);
    const removed = await event(OCT_24, recorder);
  ...
  it('classification is evaluated as of the end of the month, not as of now', async () => {
    ...
    const october = await event(OCT_3);
    ...
    const nov7 = await event('2027-11-07');
    const nov14 = await event('2027-11-14');
