...
That property is not extendable. Section 2 and decision 0206 require the **owning module** to
compute: `hierarchy` walks the placement graph, `attendance` aggregates DCC and Cell attendance,
`cells` derives the Cell coverage denominator, `people` supplies identity and lifecycle. `attendance`
may not root a query in `pastoral_assignments`, so a leader-scoped report is two statements by
construction, and coverage makes it three.
...
and a whole-church monthly aggregate is the longest read this system performs. Section 24 already
states that any unbounded wait inside a transaction is a liveness hazard; this transaction has no
unbounded wait in it — it takes no locks — but it is long, and a bound on concurrent report
computation is the thing to reach for if the pool is ever pressed, rather than shortening the
snapshot.
...
The shape stays pinned; only the shape changes. `reporting-reconciliation.spec.ts` currently asserts
one query, and that assertion becomes false the moment leader scope lands. It is **re-scoped
deliberately here rather than quietly in the slice that breaks it**, because rewriting a test that
pins a property is where this repository has repeatedly introduced defects.
