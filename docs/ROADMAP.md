...
- **Dashboard** — outstanding work above the numbers (§19), with what it counts, the value, the scope and the period on every tile, and current-state tiles never interleaved with period-based ones. It owes `GET /api/v1/cells?led_by=me`, which reaches the user's own **active** Cells. **That is not the whole of §19's meetings-awaiting-a-record list**, and the gap is named here rather than left to be found: §19 requires that list to include, in bold, a **closed** Cell's meetings while its month's window is open, and the index is `ACTIVE`-only — so those meetings stay unreachable from a screen until whatever settles §7's closed-Cell tension settles this with it, and `GET /api/v1/dcc/events/{id}/coverage-gaps`, which is the outstanding DCC work §19 puts above the figures — a coverage count with no drill-down is the "dashboard of counts" that section warns against
- **My People** (§3, §8)
