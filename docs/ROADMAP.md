...
**Why it exists as its own block.** Two completed stages shipped no interface. Stage 3 delivered Cells, Stage 4 delivered attendance, and `web/app` reaches neither — ten screens, all of them authentication, the home page, or People, against the forty routes the ledger now derives -- a figure to read off `web/scripts/check-screen-coverage.mjs`, which prints all three counts on every run, rather than off this sentence, which has now been stale three times. *The third was the index and attention routes of 2026-09-09, which added three; the sentence tells a reader to run the command and was itself left uncounted by the batch that moved the number, which is the failure it is describing.* Nothing recorded that as owed, which is the failure the ruling of 2026-09-06 closes.
...
**Nine open items in `CLAUDE.md` are deferred to these screens** and are settled here rather than rediscovered. *It said eight, and was already nine on `main` when the Cell monthly route found it -- in the sentence whose own instruction is the next one.* Count them rather than remembering: `awk '/^### Open — awaiting a ruling/,0' CLAUDE.md | grep '^- \*\*' | grep -cE 'first screens|first screen that|Decide it with them|screens block|no dashboard'`.
