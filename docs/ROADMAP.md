...
**No screen ranks, scores, or colour-grades a leader** (§17, and §22 for the response). The prohibition is easy to omit and hard to remove once leaders have seen a leaderboard, which is why it is stated here rather than only in the stage that computes the figures. *Cited to §13, §19 and Principle 8 for one commit, all three of which are narrower than the sentence: §19 says "no **dashboard** ranks leaders", Principle 8 is scoped to a score derived from **meeting status**, and §13's is under Meeting summary. §17 carries the wide form — leaders never ranked, scored or colour-coded by `NOT_HELD`, by coverage, or by any figure derived from them — and §22 forbids a rank, score or grade in any response. §17 exists partly to stop this prohibition being read narrower than it is; citing it narrowly is the mirror of that.*
...
**`web/scripts/check-screen-coverage.mjs` lands before this block, not with it**, and is the mechanism that makes the Definition of Done's interface clause fail rather than be remembered. It derives the route list from `api/src/**/*.controller.ts`, compares it against a checked-in ledger mapping each **individual route** to a screen, to a waiver naming the stage that owes one, or to a statement that no screen is owed at all — the liveness probe is called by the platform and never by a person, and waiving it to a stage would be a promise nobody intends to keep. It fails on any route the ledger does not mention *and* on any ledger entry no controller declares, and is wired into `npm run lint` in `web` beside the four checks already there. **It never reads this file**, so the per-route waiver is enforced and the paragraph naming the screens is prose — the Definition of Done says so rather than leaving the halves to look alike. The unit is the route rather than a family of them because a family entry would let a new route join an existing group unnoticed, which is this check's own failure one level down. It ships pre-populated for all thirty-five existing routes — thirteen reached by a screen, twenty-one waived, one owing none — so it is green on the day it lands and red the moment an unledgered route arrives — including during this block, when new routes appear fastest.
