...
**One item awaits a ruling, and it blocks Stage 5. Forty other things are
...
- **Whether a reschedule may move a meeting to a date whose own month has shut.** §13 fixes `reporting_month` at creation and a reschedule never moves it, so the *record* stays in the month it reports in — but `actual_date` may name a day in a month whose window has closed, and decision 0182's amendment governs *when* a record may be written rather than *which dates it may name*. Refused today by the window check already guarding the route, which is the conservative answer and may be the right one. What would settle it is a real reschedule across a month boundary late enough to straddle the 7th; §13's own example (31 January moved to 2 February) is exactly that shape and says nothing about the window.
