...
*Thirty-five distinct items across thirty-five bullets. Two arrived on 2026-08-30 when the Cell half of a Network change was settled: what remains of that question is the *membership* half and whether Cell relationships owe a term in Section 4's backdate floor, and the settled leadership half was removed from this list rather than left on it annotated, because an item awaiting no ruling does not belong under this heading. Three arrived with the leadership
...
- **Whether a Network change is refused while the person holds a Cell *membership*.** *(The leadership half of this was settled on 2026-08-30 and is in Section 4; this is what remains of it.)* Section 4's closing paragraph puts the obligation on membership as well, and Section 10 sends a reader to Section 4 for the resolution — which now contains the leadership rule and not this one. The case is not reached incidentally by the leadership refusal, because membership does not mirror pastoral assignment: an ordinary member of a Cell need not be pastorally under its leader, so refusing while somebody *leads* anything catches none of it. Correcting a member's Network today leaves them in a Cell of the other Network with nothing raised, by the mechanism Section 4 now describes. The remedy is lighter than the leadership one — ending or moving a single membership is one authorized operation, not a handover — which is exactly why it should be decided on its own terms rather than by symmetry with a rule whose cost is different. Migration 0009 escalated it as its own item and `docs/ROADMAP.md` books "Cell relationships", plural.
