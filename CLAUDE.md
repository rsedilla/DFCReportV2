...
- **Whether §7's amendment-capability gate binds a lost first submission.** §7 owes `cell.correct_subtree` to a submission that differs from what is stored, and decides every other capability before the contents can change what the caller is told. A first submission that loses a race differs from what is stored and yet amended nothing — it wrote no row at all — so neither reading is stated. It matters because the `VERSION_CONFLICT` body carries the stored present count and the submitter's name, which no read route hands that actor. **Implemented conservatively**: the capability is checked before the payload is built, so a raced actor gets the same 403 as a sequential one, and a test pins it. What is open is whether the gate is *owed* there or whether the answer should instead be that a lost first submission is exempt because it amended nothing. Raised by `architecture-guardian`, which reproduced the disclosure.
