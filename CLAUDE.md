...
- **What a closed month's Admin amendment does, and where it lives.** Sections 9, 13 and 20 all say a closed month may be amended by Admin under `records.backdate_effective_date`, with a reason, audit logged, "and invalidating that month's stored figures". No route does: `refusalFor` answers `PERIOD_CLOSED` to every actor including Admin, and `grep` finds that capability used only by pastoral reassignment, sex correction and Cell closure. Two things are unsettled rather than merely unbuilt. What "invalidating that month's stored figures" means is undefined today, because `report_snapshots` arrives with Stage 5 and there are no stored figures to invalidate — so an amendment built now would satisfy the clause vacuously and the clause would need re-reading when snapshots exist. And whether the amendment is a *third* endpoint or a flag on the two that exist is a section 22 question nothing answers. It is owed by Stage 4 rather than Stage 5: `docs/ROADMAP.md` puts "the monthly submission window **and its close**" in Stage 4, and this is the half of the close that is not a comparison.
- **Whether an attendance record may be closed with no replacement.** Section 13 requires the path: a `RESCHEDULED` meeting that ultimately does not take place "may be changed to `NOT_HELD`, preserving both records", and a `NOT_HELD` meeting carries no live attendance — so the attendance rows must be closed and nothing replaces them. `cell_attendance_supersession_is_whole` and its DCC twin (migration 0011) require a `superseded_by` wherever `superseded_at` is set, stated as an equivalence because `superseded_by` "holds the id of the row that replaced this one" and a close with no replacement looked like an incomplete write. It is not: it is the one operation Section 13 names that has no successor. The only shape the schema can express for it today is a row naming **itself**, on `cell_attendance` alone — migration 0013's contiguity trigger exempts it there by name, and refuses it outright on `dcc_attendance`, because §9 says `NOT_HELD` has no DCC equivalent and no DCC operation produces the shape. `cell_attendance_one_successor` carries the same exemption in its predicate (`superseded_by <> id`). Three answers look defensible — permit a null `superseded_by` where `superseded_at` is set, keep the self-reference and make it the documented idiom, or give the meeting-level path its own column — and none is derivable. Not blocking: nothing writes `cell_attendance` yet, **and the schema has been checked for anticipating an answer rather than merely permitting one.** The seventh `architecture-guardian` pass argued this had become blocking, on the ground that the index took a position before the question was answered. It had: with `IS NOT NULL` alone the index refused the idiom for any record already corrected once, so the path was writable only for an uncorrected one — a distinction §13 does not draw. That was a defect against the shape as it stands rather than a ruling in advance of it, and the `<> id` predicate is neutral to all three answers: under the first, `IS NOT NULL` already excludes a closed-with-null row and the clause is vacuous; under the second it is exactly right; under the third it is vacuous again. Settle it with the slice that builds Cell recording, which is the first thing that can reach the path at all.
...
- **Whether a port a module cannot bind for itself should be optional-and-refuse, or mandatory.** `CELL_RELATIONSHIPS_PORT` is injected `@Optional()` into `NetworksService`, and an unbound port refuses the Network change rather than skipping the check — the reading `CELL_SCOPE_PORT` already gives for a missing binding. Nothing in `SKILL.md` says which it should be, and the choice has a cost this repository has already paid: `test/unit/module-graph.spec.ts` exists so a wiring fault fails in seconds without a database (2026-08-24), and an optional injection defeats it — which is why binding the port in the wrong context surfaced as fifteen red end-to-end cases rather than one red unit case. A mandatory injection would work today — but not for the reason first recorded here, and the difference is the whole of what has to be settled. `@Global()` publishes a module's exports to every module *of a graph that includes it*, not to every context anywhere, so being global entails nothing on its own. It works because the one test graph omitting the binding constructs no `NetworksService`, which is "no such graph exists yet" rather than "the token is always there". The reason recorded before that, that only `AppModule` could bind one, was simply false. What optional buys is that a live deployment loses one operation instead of failing to boot, and Section 7 already takes both sides elsewhere — absent `SENIOR_PASTOR_PERSON_IDS` fails closed and the process starts, while a malformed value stops it. Settle it before a second port is declared, because the answer should be the same for both.
