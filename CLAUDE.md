...
- **Whether a Cell meets on the day it was created.** Section 10 opens a Cell's schedule row at `created_at` and says what a schedule change does at a month boundary, and it says nothing about a meeting on the creation date itself — a Cell approved on a Saturday morning whose schedule is Saturday. `GET /api/v1/cells/{id}/meetings` counts it: the derivation compares the schedule row's bounds as Manila dates, so the creation day is inside them. That reading is chosen rather than derived, on two grounds. Section 13 takes the same direction at the *other* edge, where a meeting dated the day the Cell closed "reads the Cell as it stood that day" so that a meeting the Cell actually held stays recordable; and the opposite reading refuses a record for a meeting a leader believes they held, which is the outcome section 13 works hardest to avoid. What argues against it is that a Cell approved at four in the afternoon did not exist for a meeting at ten that morning, and nothing checks the time — the comparison is deliberately day-granular, because the closing edge requires that and a rule that was day-granular at one end and instant-granular at the other would be two rules. Not blocking: the listing derives the meeting either way and no write path depends on it yet. It becomes load-bearing at the first submission, because a meeting the listing does not derive is one nothing can record — so settle it with the recording slice, and note that the same question does not arise for a schedule *change*, which section 10 pins to the first of a month where no meeting can predate the row by hours. Raised by the mutation that found the case missing: narrowing the bound to exclude the creation day passed all eleven cases of the listing suite, because the fixture created its Cell on a Wednesday.
