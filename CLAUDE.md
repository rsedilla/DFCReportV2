...
- **Whether `dcc.submit_on_behalf` must be decided before the refusal that a `CREATE` line carrying `correction_reason` has nothing to correct.** Three passages and no single answer. §9 requires that refusal to exist and says nothing about where in the order it falls; §9's ordering rule names `dcc.take_attendance` and "the amendment capability" and does not mention `dcc.submit_on_behalf`; §7's "every other capability" would include it. The code refuses first: `writeWithin` derives `outcome` from the stored record and throws `INVARIANT_VIOLATION` on a `CREATE` carrying a correction reason, both ahead of `assertMayRecord`. So one request discriminates — **409** where the person has no live record, **403** where they have one. The audience is narrow and real: an actor holding `dcc.take_attendance` over a person who is **off** their checklist and outside their `dcc.submit_on_behalf` scope, which the DCC roster does not show them and which §7 permits an Admin to grant. Under role defaults the two scopes are **equal for every role** — `OWN_SUBTREE` for `LEADER`, `WHOLE_CHURCH` for `ADMIN` and `SENIOR_PASTOR` — so the on-behalf check never refuses where the recording capability admitted, and nothing reaches the pair today. *Equality is the load-bearing fact, not the value: a version of this sentence said "both are `OWN_SUBTREE`", which is true of one role in three and would tell a reader checking `ADMIN` that this bullet is blocking when it is not.* What makes it reachable is an Admin issuing an asymmetric grant, which §7 permits. `assertMayRecord`'s own docblock sets the standard this fails — "the ordering costs nothing, so it is not left resting on which grants happen to be issued". **Framed as a violation rather than a preference, because that is what it is.** §7 states the contents-ordering rule unconditionally — "and must be" — and the DCC path breaks it as written: deriving a line's outcome from the stored `present` *is* reading contents by §7's own definition. So whoever picks this up is deciding whether to fix a violation or to write the exception §7 should have stated, not whether they like the current ordering. §7 now says this in the same terms rather than describing it as an open question about safety. Not blocking, and deliberately not settled by the Cell ruling beside it: decision 0191 covers the Cell path only, and a version of §7 that asserted the Cell ground over DCC as well was withdrawn on 2026-09-03. Settle it before slice 2c adds further paths that read a record before deciding who may touch it. Raised by `architecture-guardian` on the fix batch for that withdrawal.
- **Whether the DCC roster's per-person `record` has already fired §7's trigger for revisiting the accepted disclosure.** §7 accepts one bit — whether a submission matched what is stored — on a bound of 2^N submissions, and names what would change it: "a route that reads per-person attendance". That clause said "which none does today" of the whole system until 2026-09-03 and was false when written. `GET /api/v1/dcc/events/{id}/roster` returns each person's `present`, `version` and `recorded_at` under `dcc.take_attendance`, a recording capability that §7 does not permit to be granted `read_only` — so for DCC the bit is free and has been since the DCC recording slice, while for a Cell meeting it still costs 2^N because the Cell roster returns the members and the meeting and never who was present. §7 now states both halves and declines to draw the conclusion. What is open is which conclusion: that the acceptance was always a Cell argument and should say so, that the DCC roster should stop returning `present` to an actor holding only the recording capability, or that the disclosure is acceptable in both domains for reasons the section has not given. It shares a fact with the bullet above and answering either alone would settle the other by accident.
