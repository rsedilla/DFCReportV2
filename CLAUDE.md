...
**One item awaits a ruling, and it blocks Stage 5. Thirty-five other things are
...
- **What "the period being viewed" means when a Cell resolves an audit entry.** Section 7 resolves an audit entry through its target and resolves a Cell "through the Cell's leader as of the period being viewed, falling back to its last leader where the Cell is closed". A report asks about a period; an **audit entry is an instant**, so the phrase has two readings and Section 7 fixes neither. Read as the entry's own instant, a Cell-targeted leadership entry resolves through whoever led the Cell when it was written — which for `cell_leadership.opened` is the person the entry names, so it never diverges from a person target. Read as *now*, it resolves through the Cell's current leader, which is what every other Cell-scoped read in the application does and which diverges after any handover. Both readings are defensible and they disagree about who may read a past leadership entry. Not blocking, and the distinction is worth keeping: **writing** an entry is identical under both, so the 2026-08-31 target ruling stands either way and Stage 4 may write freely; what the answer binds is the first route that **reads** the log, and `audit.view` has none. Settle it with that route, and note that the same phrase governs every other Cell-resolved target, so the answer is not the audit log's alone. Raised by `architecture-guardian`, after two drafts of that ruling asserted a divergence mechanism and each was refuted by the other reading.
- **Whether the three keyset cursors should share an encode/decode pair.** `people.controller.ts`, `roster-cursor.ts` and `leadership-request-cursor.ts` each carry their own base64url-JSON encoder and a decoder that shape-checks its keys, and they now share two things: the length bound and, since 2026-08-31, the refusal. What is not shared is the pair itself, and the argument for generifying it is the history of the three — the roster cursor shipped with a key that could not be planned at all, the queue cursor shipped with `Date.parse` where PostgreSQL's parser was meant, and the unreadable-cursor behaviour was one decision copied twice that read as three agreeing. The argument against is that the three keys differ in kind (two names and a UUID, two names and a Member ID, an instant and a UUID) and one of them needs a format predicate the others do not, so a generic pair is a type parameter plus a per-route validator, which is most of what each file already holds. Not blocking, and Stage 4 adds a fourth — settle it there, with four in front of whoever decides rather than three.
