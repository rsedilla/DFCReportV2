...
- **aarchitecture-guardian** — read-only review of a change against `SKILL.md` domain invariants. Reports findings; does not edit.
...
## Mandatory Review
A change **must** receive `aarchitecture-guardian` review before it is considered complete when it does any of the following:
...
**Run `aarchitecture-guardian` before requesting human review**, not after. If the change meets any Mandatory Review condition above, run it yourself and resolve what it reports. Arriving at review with its findings already addressed is the point of having it; asking a reviewer to discover them is not.
...
closed the same day by `aarchitecture-guardian` passes.
...
- **What an API does with a date-only value that is well-shaped and is not a day.** Section 22 fixes the format — "date-only fields are plain `YYYY-MM-DD` and are always Asia/Manila dates" — and nothing says what happens to `2026-02-30`, which matches that and is not a date. The consequences are already in the tree rather than hypothetical. The predicate deciding it was written **four times** in four modules before anyone noticed the first three (`people.shared.ts`, `tree-import/tree-csv.ts`, and the capability guard's own, which is what surfaced this), and two of them disagreed about years 1 to 99; they are now one function in `common/time/manila.ts`. Two DTO conventions exist for the same field type: `people.dto.ts` documents `@Matches` plus `@IsDateString({ strict: true })` and uses it on every date it takes, while `cell-meetings.dto.ts` and `cells.dto.ts`'s `effective_date` had the shape check alone. And the failure mode differs by route rather than by rule: a well-shaped impossible `{meeting_id}` reached a `::date` cast and answered `INTERNAL_ERROR`; the same value as a `month` answered a plausible listing for the wrong month, then a 500, then a 422 as each was fixed; and `CloseCellDto.effective_date` still passes `2026-02-30` to `startOfManilaDay`, which normalises it to **2026-03-02** and writes a Cell closure effective on a day nobody named, into an effective-dated history table, with no error at all. That last one is the worst of the three and is the one nothing has caught. What would settle it is one sentence in Section 22 — a date-only field that is not a day is refused with `VALIDATION_FAILED` at the edge, by one shared predicate — and applying it to every date-only DTO field. Not blocking the routes already written, each of which now refuses correctly; it is blocking in the sense that the next date field added will get whichever convention its author happens to copy. Raised by `aarchitecture-guardian` across two passes on the dated Cell-meeting scope.
- **Whether a meeting-scoped roster *read* follows Section 7's write rule or Section 13's read rule.** Section 13 states a split in one sentence: a Cell meeting resolves "through the Cell's leader **as of the period being viewed** for a read, and through the current leader for a write — except for a write against a **closed** Cell, which resolves through whoever led it on the meeting's date... while the month's window is open." Section 7's general sentence agrees. But Section 7's exception bundles "the meeting-scoped roster read that write requires" *with* the write, inside a clause introduced as "A closed Cell has one exception" — so for a closed Cell the two readings coincide, and for an **ACTIVE** Cell they disagree. `leaderForMeetingScope` is one method serving both `GET .../roster` and `POST .../submit`, so it gives both the write answer, and a leader who handed on an active Cell is refused the roster of a meeting Section 13 appears to place in their scope. Nothing says which reading wins. Three answers look defensible: split the resolution in two, so the read resolves by date on every Cell state and the write keeps the current-leader rule on an active one; keep one resolution and amend Section 13 to say the roster read follows the write it serves; or keep one resolution and accept that the read is narrower than Section 13 reads, stated as such. **This is the first dated read the system has**, which is the trigger the audit-log item below names — it says the phrase "governs every Cell-resolved target rather than the audit log alone" and "settle it with the first dated read". Not blocking the code already written: the behaviour on an active Cell is unchanged from before the dated resolution existed, and is pinned by a case so a ruling either way has something to move. Raised by `aarchitecture-guardian`, which found the port asserting the asymmetry was "section 7's and deliberate" — a settlement the specification does not make.
- **Which leader a meeting belongs to when the Cell changed hands on the meeting's own day.** Section 13 fixes the instant as "the meeting's `actual_date` where it has one, and its `scheduled_date` otherwise", and settles week-versus-day at length — a week "is not an instant, and a handover or a closure may land on any day inside one". It says nothing about a handover landing on the *same* day as the meeting. Both `leaderOnDateWithin` and `leaderForMeetingScope` compare Manila **dates**, which section 13 requires at the closure boundary so that a meeting held the day a Cell closed stays recordable — and the cost of that comparison is exactly here: on a handover day the outgoing and incoming leadership rows both cover the date, and the query cannot tell which of them was in the room. It currently answers with the later-starting row, which is an ordering chosen for a different purpose rather than a rule anybody decided, and it is wrong whenever the handover was recorded *after* the meeting took place. Three answers look defensible and none is derivable: compare at the meeting's scheduled instant rather than its date, which the port would have to read from `cell_schedules` and which reintroduces the closure problem section 13's day-granularity solves; prefer the outgoing row, on the ground that a handover is usually recorded after the meeting it follows; or state in section 13 that a handover takes effect at the start of the following day, which would make the comparison exact and is the shape section 10 already uses for a schedule change. **It decides reporting attribution, not only a scope answer, and the first version of this bullet said the opposite.** That version called it non-blocking partly because "every other path reads the frozen `responsible_leader_id` rather than re-resolving", which is false of both: `CellMeetingsService.rosterFor` re-resolves whenever the meeting has no row yet, and `submit` re-resolves to decide the value it **freezes**. Section 13 makes that value the leader a meeting rolls up to for Sections 12 and 20 — so a handover on a meeting's own day writes a permanent attribution chosen by a `started_at DESC` tie-break, and a rule that decides where attendance is counted is one Section 13 must state rather than one an `ORDER BY` should. Raised by `aarchitecture-guardian`, which also refuted the sentence that had classified it. Still not a Stop Condition for the code already written — nothing reaches it without a handover and a meeting on one day, and the fixture in `closed-cell-meeting-scope.e2e.spec.ts` deliberately stays a day clear — but it is owed before Stage 5 reports a month, because by then the wrong attribution is in closed data that Section 20 forbids moving. Found by that suite failing on its first run, which is how the ambiguity became visible at all.
- **What period a read of the audit log is asking about.** Section 7 resolves an audit entry through its target and resolves a Cell "through the Cell's leader as of the period being viewed, falling back to its last leader where the Cell is closed" — and it *does* define that phrase, under **An effective date does not move the scope decision**: "the period a read is asking about", as against a write, which "is acted on now". What is open is one layer in from there. A report asks about a month; a read of this log might be a single entry, which is an **instant**, or a filtered range, which is a range. Under the first, a Cell-targeted leadership entry resolves through whoever led the Cell when it was written — for `cell_leadership.opened` that is the person the entry names, so it never diverges from a person target. Under the second it resolves through whoever led the Cell across the range asked for. The two disagree about who may read a past leadership entry. Not blocking, and the distinction is worth keeping: **writing** an entry is identical under both, so the 2026-08-31 target ruling stands either way and Stage 4 may write freely; what the answer binds is the first route that **reads** the log, and `audit.view` has none. Nor is the code an answer yet — `CellsReadService.leaderForScopeWithin` resolves the latest leadership row and every Cell-scoped path goes through it, but `cell-scope.port.ts` says in terms that this is "the undated case rather than a claim that dates do not matter". *It also said a past-period read "will need a dated variant", and the branch of 2026-09-02 built one -- so that half of the quotation now survives in that file only inside the retraction recording that it was left standing one commit too long. The trigger this bullet names has therefore fired: the meeting-scoped roster read is the first dated read, and the bullet above it is where that question now sits.* Settle it with the first dated read, and note that the phrase governs every Cell-resolved target rather than the audit log alone. Raised by `aarchitecture-guardian` after two drafts of that ruling asserted a divergence mechanism and a third called Section 7 silent.
- **Whether a person may record their own DCC attendance.** Section 9 makes a person's attendance their *direct pastoral leader's* obligation, so a person is never on their own checklist — the submitter walk starts at their leader. But `dcc.submit_on_behalf` at `OWN_SUBTREE` covers the actor themselves (`scopes.ts`: "The actor's pastoral subtree, including the actor"), so a line naming the actor is admitted today, written with their leader as responsible leader and themselves as `recorded_by`, and it completes their leader's coverage. Section 14 permits recording "on behalf of a downline leader within their pastoral subtree" and says nothing about oneself; section 5 has an explicit prohibition on acting on oneself for pastoral assignment and section 9 has no counterpart. Whether that is right is a pastoral question rather than a technical one: section 9 rests recording on "a leader knowing who was in the room", and self-reporting is a different basis. Not blocking — the record is honest either way and nothing is corrupted — and decision 0172 deliberately declined to settle what an actor may submit for beyond their own roster. Raised by `aarchitecture-guardian` on the DCC recording slice.
