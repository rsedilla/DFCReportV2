...
them one at a time is as many chances to miss the next. Enforced where an account's
effective authority is assembled, beside the `read_only` rejection it mirrors — same
argument as §2 gives for the guard being declarative: an operation that forgets the
check looks exactly like one that does not need it. The set is **derived** from the
role catalog rather than transcribed beside it, so a capability added at Whole
Church is protected in the change that adds it, and a hand-kept list cannot go stale
in the direction that silently stops protecting something.
