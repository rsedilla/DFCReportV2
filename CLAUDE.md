...
- **Whether §7's amendment-capability gate binds a lost first submission.** §7 owes `cell.correct_subtree` to a submission that differs from what is stored, and decides every other capability before the contents can change what the caller is told. A first submission that loses a race and **disagrees** with what is stored amended nothing — it wrote no row at all — so neither reading is stated. It matters because the `VERSION_CONFLICT` body carries the stored present count and the submitter's name, which no read route hands that actor. **Implemented as: the disagreeing branch is gated, the agreeing branch is not.** The agreeing half is not open at all — §7 settles it, because a submission matching what is stored is not an amendment and owes no amendment capability, and §22 now says so among its guards. What is open is only the disagreeing half: whether the gate is *owed* there, or whether a lost first submission is exempt because it wrote nothing whatever it carried. *A version of this bullet said the capability "is checked before the payload is built, so a raced actor gets the same 403 as a sequential one". That was written of an implementation which checked it unconditionally, which refused the agreeing actor a write §7 permits — 403 raced against 201 sequential, the mirror of the defect being fixed — and stored the refusal against the idempotency key so a conforming retry replayed it for ever. Both the sentence and the code it described were wrong, and the bullet's premise that such a submission "differs from what is stored" was false of exactly the case that broke.* Raised by `architecture-guardian`, which reproduced the disclosure and then reproduced the over-correction.
