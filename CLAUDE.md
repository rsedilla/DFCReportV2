...
**Three items await a ruling. Two of them block Stage 3 and are listed first; the
third blocks Stage 5. Twenty other things are unsettled, none of them blocking. They are listed at the end, so this section is the
whole of what is open.**

**How a Cell passes from one leader to another while it stays open.** No section
defines it. `cell.manage_leadership` sits in Section 7's closed list with nothing
saying what it may do, and Section 13 refers in passing to a genuine handover as "a
separate, deliberate change to `cell_leaderships`". Section 11 now requires that
change to be one transaction, and says nothing about who may make it.

The ruling of 2026-08-28 proposed request-and-approve, on Section 10's own argument
that the workflow exists so no leader decides alone that one of their own disciples
should lead, and was withdrawn because settling it needs three answers it did not
give. **Which capability** guards the request and the approval: reusing
`cell.request_creation` and `cell.approve_creation` makes both names misdescribe what
they guard, and a second pair adds two names to a list Section 7 declares closed.
**Which of two people** the guard resolves against: a creation has one target, a
handover has an outgoing leader and an incoming one, and they need not share a branch,
since Section 10 says Cell membership need not mirror pastoral assignment. And
**whether one pending request per prospective leader still holds** once two kinds
share a table: that index exists because two new-Cell requests are indistinguishable
downstream, which is not true of a handover naming its Cell — while the constraint
that reasoning does support is absent, since nothing would forbid two pending
handovers of the same Cell to two different people.

Section 3 makes this more than a Stage 3 convenience: it offers handing the Cell on as
one of two ways to unblock archiving a Person who leads a Cell, and
`cell.request_creation` is `SUBTREE_EXCL_SELF` for every role, so an Admin outside the
pastoral tree — a state Section 5 invariant 3 explicitly provides for — could not
submit the request at all.

**What reversing a closure does to a Cell's leadership, category and schedule.**
Section 10 says reversing a closure recorded in error is an Admin correction requiring
a reason and an audit entry, and says nothing about the three rows the closure ended.
Two of the rulings of 2026-08-28 meet on this operation: an `ACTIVE` Cell must hold
exactly one leadership row, and a schedule row must start on a first of month or at
the Cell's `created_at` — so a mid-month reversal satisfies neither and aborts at
COMMIT rather than answering, which is the 500-instead-of-an-answer failure this log
records five times.

Three answers, and the third is the cheapest. **Refuse the reversal** and require a
new Cell, which is what Section 10 already prescribes one paragraph above for a
ministry that restarts. **Reopen the ended rows**, on the reading that a closure
recorded in error did not happen — which conflicts with Section 5's rule that a row
is never overwritten in place, and, more seriously, moves months already reported: a
Cell closed through March and April had no meetings and no members, and reopening its
schedule and membership rows retroactively gives those months a denominator, against
Section 3's reproducibility guarantee. Or **open new rows** at the reversal date,
which is honest about the period the Cell spent closed and needs the schedule trigger
to admit a third case.
