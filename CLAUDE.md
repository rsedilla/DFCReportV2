...
**One item awaits a ruling, and it blocks Stage 5. Forty-one other things are
...
- **What bounds a reschedule's `actual_date`, beyond the two refusals now written.** §13 states exactly one: a meeting dated after the Cell's closure is refused, "the Cell did not exist to meet". The code adds a second — the month it moves into must be open — chosen while fixing a defect rather than derived from the specification. Nothing says whether an actual date may precede the scheduled one (a meeting held *early*), precede the Cell's creation, or fall arbitrarily far ahead, and the route applies none of those three. Not blocking: each is refused by nothing today and reachable only by a leader who holds the capability and types a wrong date, which the change history then records. Settle it when a real reschedule needs a bound the code does not have — and note that §13's own example, 31 January moved to 2 February, is a **forward** move across a month boundary, so a naive "must not precede the scheduled date" would be the easy wrong answer. Raised by `architecture-guardian` on the transitions slice.
...
- **Whether §7's amendment-capability gate binds a lost first submission.** §7 owes `cell.correct_subtree` to a submission that differs from what is stored, and decides every other capability before the contents can change what the caller is told. A first submission that loses a race and **disagrees** with what is stored amended nothing — it wrote no row at all — so neither reading is stated. It matters because the `VERSION_CONFLICT` body carries the stored present count and the submitter's name, which no read route hands that actor. **Implemented as: the disagreeing branch is gated, the agreeing branch is not.** The agreeing half is not open at all — §7 settles it, because a submission matching what is stored is not an amendment and owes no amendment capability, and §22 now says so among its guards. What is open is the disagreeing half: whether the gate is *owed* there, or whether a lost first submission is exempt because it wrote nothing whatever it carried. **And it is open across both domains, with the two implemented differently today** — the Cell path gates that branch, and `DccAttendanceService.conflictAfterLostRace` does not, so an actor holding `dcc.take_attendance` alone is answered `409` carrying the stored value *and the recording account's name* where the identical body sent sequentially answers `403 dcc.correct_subtree`. The DCC roster publishes neither of those, so the §7 disclosure argument that settled the Cell path applies unchanged and this looks like a defect rather than a deliberate difference; it is not fixed in the branch that found it, because that branch changed the Cell path and CLAUDE.md asks a pull request to be one coherent change. Whoever settles the question fixes DCC in the same ruling, or states why the two domains differ. *A version of this bullet said the capability "is checked before the payload is built, so a raced actor gets the same 403 as a sequential one". That was written of an implementation which checked it unconditionally, which refused the agreeing actor a write §7 permits — 403 raced against 201 sequential, the mirror of the defect being fixed — and stored the refusal against the idempotency key so a conforming retry replayed it for ever. Both the sentence and the code it described were wrong, and the bullet's premise that such a submission "differs from what is stored" was false of exactly the case that broke.* Raised by `architecture-guardian`, which reproduced the disclosure and then reproduced the over-correction.
