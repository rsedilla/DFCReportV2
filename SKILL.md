...
**For a Cell meeting** it is accepted rather than closed, and the bound **was** why — was, because the paragraph below retracts it and says what the acceptance rests on instead. The submission must name every member of the meeting exactly once (Section 13), the answer covers the whole set with no per-line detail, **and while nothing else published the marks** recovering N people's attendance **cost** 2^N submissions rather than N. *The second premise is stated because it is the one that died. The submission's shape is unchanged, so the conclusion is past-tensed on the loss of the premise rather than on any change to the route — and two earlier attempts each drew it from premises that do not yield it: one scoped the conclusion to the submit route and past-tensed it together, where through that route the cost is what it always was, and one past-tensed the unqualified conclusion while stating the first premise alone.* *The domain is named because that shape is a fact about a Cell submission and never held for DCC, which the bullets below give its own ground; an unqualified "it" here asserted the bound generally and is withdrawn by the DCC bullet below that names the domain.* And every actor who reaches it holds the capability that records this record and may overwrite it outright by holding one more — which is a ground in its own right, one of the two decision 0191 has left, and is untouched by the bound going.
