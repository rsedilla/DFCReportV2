...
- A submission whose **status** differs from the committed one is refused before the comparison, exactly as it is refused when nothing raced (Section 13: a status change is a separate operation). It cannot take either outcome — a `NOT_HELD` body carries no attendance, so the comparison would find nothing to disagree with and answer `RESOURCE_BUSY`, whose meaning is that the identical body resubmitted succeeds. It would not.
- The **amendment capability** is required of the disagreeing branch before its conflict is built, because that conflict carries the stored figures. It is *not* required of the agreeing branch, which writes nothing and owes no amendment capability under the rule in Section 7 — asking for it there refuses a write this specification permits, and stores the refusal against the idempotency key.

*Each of the three was a live defect first and a sentence afterwards, on the Cell first-submission race in 2026-09-03: the uniqueness violation surfaced as a 500, the status disagreement answered `RESOURCE_BUSY` with a retry that could never succeed, and the amendment capability was first absent and then demanded of everyone. The last two were introduced by the fixes for the ones before them.*
