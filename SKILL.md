...
**For a Cell meeting** it is accepted rather than closed, and the bound is why. The submission must name every member of the meeting exactly once (Section 13), and the answer covers the whole set with no per-line detail — so recovering N people's attendance costs 2^N submissions rather than N. *The domain is named because the bound is a fact about a Cell submission and does not hold for DCC, which the paragraphs below give its own ground; an unqualified "it" here asserted the bound generally and was withdrawn six lines later.* And every actor who reaches it holds the capability that records this record and may overwrite it outright by holding one more.

**What would change this is a route that reads per-person attendance, and the answer differs by domain.** This paragraph said "No *Cell* route does" until the ruling of 2026-09-09, and **`GET /api/v1/cells/{id}/meetings/{meeting_id}/roster` now carries each member's mark**, so the 2^N bound above is gone and what it was supporting has to be re-derived. It is, here, rather than inherited — which is what the withdrawn sentence asked of the first surface to do this. *The meeting resolves through its **current** leader on an `ACTIVE` Cell and through the frozen responsible leader only on a closed one whose window is open, as the bullets above state; a first version of this re-derivation named the frozen leader unconditionally.*
...
So the two acceptances rest on different grounds, and they are two acceptances rather than one: **publishing the marks** rests on the three Cell grounds re-derived above, and **the confirmation bit a lost race answers** rests on the two decision 0191 has left — the actor is not a stranger to the record, and refusing an unchanged submission costs an honest leader a resubmission. DCC has its own three, below. Stated separately rather than merged, which is what the merged form got wrong: it counted one enumeration while naming the other. *This said "one for a Cell meeting" and gave that one as the 2^N bound, in the present tense, eighteen lines below the re-derivation that declares the bound gone and replaces it with three. A Cell meeting's bit **cost** 2^N, because the submission names every member at once and is answered as a whole, until the roster began carrying the marks on 2026-09-09.* DCC is recorded per line with per-line outcomes, so no such bound exists there, and none is needed: the version is required to be published at all, and the figures are reachable in N probes whether or not the roster shows them. *This clause said "which none does today", of the whole system, and was false when written; a version that noticed left the consequence open and is now settled.*
