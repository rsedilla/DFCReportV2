...
**A collection is a target too, and one whose period dates its figures resolves its membership as of that period** (ruling of 2026-09-10). The trigger above is what the request *names*, not what shape the answer takes, so a route returning a list of Cells under `cell.view_subtree` and taking a required `month` is a dated viewing read exactly as a single-target read of the same month would be. Its membership resolves at **the period's final millisecond** — decision 0218's instant, adopted rather than a second one derived — so a Cell handed over after the period ended lists for the leader who oversaw the period, and not for the one who holds it today. Two things this does not do, both stated because each was assumed once. It does **not** give a mid-period handover to both leaders: that period resolves through whoever held the Cell when it ended, which is the same answer the base bullet gives a `CELL` report selector and the same one the current-state rule gave. And it does **not** date the detail routes beneath it, each of which is undated for a reason of its own rather than as a class: `GET /api/v1/cells/{id}/meetings` carries a **recording** capability, which the rule above leaves undated whether the route reads or writes, while `GET /api/v1/cells/{id}/members` carries this same **viewing** capability and is undated because it names no period — the opening clause of this heading rather than an exception to it. An index and the route a row links to are meant to resolve differently, so a divergence between them is not evidence that either is wrong. What that divergence costs on one screen is the open question about the closed-Cell fallback under a write-resolving capability, and it is not settled here.
