...
**And a third option exists that nobody has costed**, which is why this is not settled here: **this section** says a meeting's roster is guarded by "`cell.take_attendance` for a submission and `cell.correct_subtree` for a correction" — in its capability-boundaries list, above — while the route declares the first alone. *That sentence was attributed to Section 22 for one commit. It is this section's, which makes the divergence sharper rather than softer: Section 7 disagrees with itself and with the route, rather than with a neighbouring section.* Carrying `record` only for an actor holding the second is a reading this section can support and this specification has not chosen. Which capability may read a Cell meeting's recorded marks is **recorded as open in `CLAUDE.md`**: two live statements give two answers — the capability-boundaries list above, and the route — and the field exists to serve the correction case. *A third stood in this very paragraph until it was withdrawn, and this sentence went on counting it.*
...
may not change.* Under which capability those marks may be read is recorded as open in
`CLAUDE.md`. It is decision 0194's
