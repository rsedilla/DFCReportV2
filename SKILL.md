...
**Two lock classes, and they are ordered against each other.** Ordering advisory locks among themselves is not enough once an operation takes row locks as well. A person advisory lock is taken first, in ascending lock key; every row lock the operation needs is taken after them, in ascending identifier. Both classes, in that order, before the operation decides anything.
...
An operation that must know which people to lock before it can lock them reads that list first, outside the lock. The read may be stale by the time the locks are held, and that is safe for a stated reason rather than by luck: the row lock taken afterwards serializes against any writer that added somebody in between, so their work waits and this operation's own checks run against a settled state. An operation relying on that ordering says so where it is written.
...
**The effective date has a floor, and below it no write satisfies the constraints.** It is the latest of: the start of every row still open on the Cell in **every effective-dated table this closure ends** — membership, leadership, category and schedule — and the end of every such row already closed.

The floor covers all four because the closure writes all four, and the first version of this rule named only two. Category and schedule carry the same `ended_at >= started_at` check as the other pair, and theirs is an immediate table check rather than a deferred trigger, so it raises at the statement. That matters more than it sounds: a schedule change takes effect on the first of the following month and a category change on the day it is made, so either can be the **latest** row on a Cell whose memberships and leadership are much older — and a closure backdated past it is refused by a constraint the stated floor could not see.
...
An earlier version of this paragraph prescribed the Cell locks alone and said the ordering removed the cycle. It removes the cycle between two closures, and it *creates* one against an ordinary membership move, which takes the person lock first and the Cell row lock at commit. The rule is Section 5's because it spans two lock classes and Section 5 owns the locking discipline; what remains here is that a closure obeys it.

The members to be locked are read before the Cell is locked, which Section 5 permits and explains: the Cell row lock taken afterwards serializes against anybody adding a member in between.
