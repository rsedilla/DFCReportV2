...
**Enforced by refusing the shape, not by comparing two instants.** The trigger refuses a `dcc_attendance` row naming itself as its own successor because it is one. Stated because the first version enforced it as a side effect: a self-referenced row was compared against its own `recorded_at`, which differs from its own `superseded_at` on any close of non-zero length — so a close whose two ends were the same instant compared equal and raised nothing. What that reached was narrow, and the qualifier matters because it is the same one the Cell defect turned on: for a record **already corrected once**, the predecessor and the self-closing successor carry the same `superseded_by`, and `dcc_attendance_one_successor` refused it. The gap was a first record, closed at zero length, and it was still a case where the premise this paragraph rests on held because nothing had written the row rather than because anything refused it.
