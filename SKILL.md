...
So the acceptance above rests on different grounds in the two domains — one for a Cell meeting and the three above for DCC — stated separately rather than merged. A Cell meeting's bit costs 2^N because the submission names every member at once and is answered as a whole. DCC is recorded per line with per-line outcomes, so no such bound exists there, and none is needed: the version is required to be published at all, and the figures are reachable in N probes whether or not the roster shows them. *This clause said "which none does today", of the whole system, and was false when written; a version that noticed left the consequence open and is now settled.*
