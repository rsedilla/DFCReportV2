...
**For a Cell meeting** it is accepted rather than closed, and the bound **was** why — was, because the paragraph below retracts it and says what the acceptance rests on instead. The submission must name every member of the meeting exactly once (Section 13), and the answer covers the whole set with no per-line detail, so recovering N people's attendance **cost** 2^N submissions rather than N. *The domain is named because that shape is a fact about a Cell submission and never held for DCC, which the paragraphs below give its own ground; an unqualified "it" here asserted the bound generally and was withdrawn in the paragraph above.* *The claim is past-tensed **unqualified**, which is the form it was asserted in and the form that died: through the submit route alone the cost is what it always was, and what a second route publishing the marks destroyed is the claim that N marks are expensive to obtain at all. A version scoped to "this route" and past-tensed together said nothing coherent, each half refuting the other.* And every actor who reaches it holds the capability that records this record and may overwrite it outright by holding one more — which is a ground in its own right, one of the two decision 0191 has left, and is untouched by the bound going.
