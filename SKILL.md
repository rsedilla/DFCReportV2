...
**The effective instant is read after the lock, not before it.** An operation that stamps first and locks second carries an instant from before it waited, so the winner commits a row whose `started_at` is later — and the loser, comparing its own stamp against a floor computed from that row, is refused as too early. That refusal is a rule about what may be recorded, so it answers with a 4xx, and Section 22 stores a 4xx against the idempotency key and replays it for the whole retention. A request that was legal when it arrived is then refused permanently for having waited, which is the dead end Section 22's store-and-release split exists to prevent.

**And a floor breached with no effective date supplied is contention, so it answers `RESOURCE_BUSY`.** The instant it collided with was chosen by the system rather than by the caller, and nothing about the request was wrong; the retry Section 22 advises is the correct advice and a 4xx is what makes it impossible to take.

Both are stated here because deriving them has not been enough. The first was worked out once, written into one operation's comment, and the operation beside it kept the defect for six days with the whole suite green; the second was written into that same comment and its sibling disagreed with it until 2026-08-29. Every write path that takes this lock owes both, and each new one has met them by rediscovery rather than by reading.
...
**Ordering advisory locks against row locks is not settled, and one operation already takes both.** A Cell membership write takes an advisory lock on the person and then, at commit, a row lock on the Cell, because the deferred trigger checking that Cell's state reads it `FOR SHARE`. So the order is already fixed by an existing writer, and anything added later must be established against it rather than chosen freely.

What is missing is the rule, not the need. Section 10's Cell closure is the operation that makes the absence bite, because it takes both classes deliberately rather than incidentally and can therefore take them in the opposite order.

That is recorded as a gap rather than filled because three orderings were written for it and each was refuted, the last by reproducing a deadlock. Two properties defeat reasoning about it on paper. A deferred constraint trigger takes row locks at commit, in the order rows were written, which no rule reaches after the fact. And an operation that locks people must first know which people, so putting the row lock first means reaching back for advisory locks while holding it, and putting the advisory locks first means deciding them from a read that another transaction can invalidate before the row lock is taken.

An operation needing both classes therefore states its ordering, its lock strengths, and what bounds each wait, and demonstrates them against concurrent writers rather than asserting them. This section records the rule once one holds.
...
**No row of a closed Cell may end after the Cell did.** The rule holds for leadership and membership rows, and it reaches rows closed by an earlier handover as well as the ones this operation writes — the database enforces exactly that today. **Whether it extends to category and schedule rows is open**, and it is what the closure endpoint settles with the constraint it adds. Those two joined the closure's writes on 2026-08-29 and the rule was never restated for them, so the reach is genuinely undecided rather than merely unenforced.

**The effective date needs a floor, and this specification does not yet state one.** A closure ends every open row on the Cell at the effective date, and `period_ordered` on all four tables refuses a period that ends before it starts — so some dates are satisfiable by no write at all, and an operator submitting one meets a raw constraint violation rather than an answer. That much is settled. What the floor **is** is not.

Three formulations were written and each was wrong, which is why this is recorded as open rather than answered a fourth time. The difficulty is a schedule change: it takes effect at the start of the following month, so both the incoming row and the outgoing row it closes carry timestamps in the **future**, and any floor reading them sits in the future too — leaving a Cell that has merely been rescheduled closable by nobody, since a forward-dated closure is not an operation this specification defines. Excluding rows that have not started yet does not reach the outgoing row, which has.

It cannot be settled here because it depends on the question stated just above: whether the outlive-closure rule reaches category and schedule rows, or only the leadership and membership rows the database constrains today. The closure endpoint settles both together, against the schema rather than against prose, and states the floor then.

Until then the refusal an operator receives is undefined, and that is the gap rather than a licence: an implementation that meets an unsatisfiable date owes an answer naming the earliest legal one, on the pattern Sections 4 and 5 already use.

**Whatever the floor turns out to be, it says how far back a closure *can* be dated; `records.backdate_effective_date` says who may date it back at all.** The two are independent, and this half is settled. Any effective date earlier than the current day requires that capability (Section 7), which is Admin's. The closer may always date a closure today, so nothing is blocked — what they give up is an accurate scheduled-meeting count for the days between the Cell's last meeting and the closure being recorded.
...
**How a closure takes its locks is not settled here, and it is the other thing the closure endpoint must establish rather than inherit.** A closure changes a Cell's state and writes a membership row per dispersed person, so it needs both of this system's lock classes — the advisory locks on people that Section 5 governs, and row locks on the Cells themselves. A membership write already takes both, in a fixed order, so a closure's ordering has to be established against that existing writer rather than chosen.

Three orderings were written for it and each was refuted, the last by reproducing a deadlock against the database. The reason they failed is worth stating so the fourth is not attempted the same way: a deferred constraint trigger takes row locks at commit, in the order rows were written, which no rule reaches after the fact; and a closure cannot know which people to lock until it has read its member list, so any ordering that puts the Cell first must then reach back for people, and any ordering that puts people first reads a list that can be stale.

This is a mechanism rather than a rule, and prose is the wrong instrument for it — each of the three attempts read as sound and one of them deadlocked. The closure endpoint establishes the ordering, the lock strengths, and what bounds each wait, verifying them by execution against concurrent writers, and Section 5 records the result once it holds.
