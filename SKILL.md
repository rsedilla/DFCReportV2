...
**No row of a closed Cell may end after the Cell did.** That reaches every effective-dated table this closure ends, and rows closed by an earlier handover as well as the ones this operation writes. The database enforces it today for leadership and membership rows; whether it reaches category and schedule rows is open, and the closure endpoint settles that with the constraint it adds.
