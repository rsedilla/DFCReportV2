...
**A lost race has three outcomes, and this governs every lost race rather than only the two above.** The loser re-reads the committed state and answers on what it finds — which is not the same question as what the winner wrote, because the loser holds no lock while it re-reads and any number of writes may have landed first.
...
- Anything else is a defect and keeps failing loudly. Letting the uniqueness violation surface on its own would answer `INTERNAL_ERROR` on an ordinary race, which is what naming these cases is for.
