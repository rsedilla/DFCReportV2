...
**Contents, rather than every stored value, and the narrowing is not cosmetic.** This said "before what is stored is read" for one commit on 2026-09-03, and that is false of both implementations: the Cell path reads the meeting row and refuses a status change with `current_status` in the body, and the DCC path derives a line's outcome from the stored record, each of them before the on-behalf check. Neither is the defect above, because what those refusals expose is the meeting's own status, version and submitter — which `GET /cells/{id}/meetings/{meeting_id}/roster` hands the same actor under an *identical* capability and target declaration. The rule is about the per-person attendance that a recording capability does not otherwise publish, and stating it as "anything stored" made it a sentence no implementation obeyed, which is worse than a narrower one they do.

**That rests on the roster read carrying the identical declaration, and it is checked rather than assumed.** If that read is ever given a viewing capability of its own — which this specification records as an open question about every Cell-scoped read at once — the two routes stop admitting the same actors, and the ordering above has to be re-derived rather than inherited. `test/unit/capability-scope-resolution.spec.ts` asserts the two declarations are identical, so the change that separates them reddens a test instead of quietly widening what a refusal answers.
...
**What would change this is a route that reads per-person attendance**, which none does today: `GET /api/v1/cells/{id}/meetings/{meeting_id}/roster` returns the members and the meeting, never who was marked present. The first surface that offers those figures makes the bit free, and whoever builds it revisits this rather than inheriting it.
