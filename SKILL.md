...
Each term is a maximum over rows that may be empty, and an empty term contributes nothing. A Person encoded but not yet assigned has no rows at all, so their floor is unbounded and a correction may be backdated freely — there are no edges to strand. A Network root contributes nothing from term (a) and is bounded by its downline edges through (b) and (c), which is what its own case needs.

The floor is deliberately written over **rows with a leader**, so it does not depend on how a root is represented. Section 5 describes a root both as having no active assignment and as having an assignment with a null `leader_id`, and the two readings disagree about whether a row exists; that ambiguity is recorded as open in `CLAUDE.md`. It does not reach this rule either way, because a row with a null `leader_id` is passed without comparison by the same-Network trigger and therefore can never be an edge the correction has to strand.

At or above the floor every selected edge can be resolved. Each open edge began at or before the effective date, so it can be closed at that instant, and an edge closed at exactly the effective date is no longer selected. This is the same one-instant operation as above, performed for the person's own assignment and for each disciple they lead.

**The two sides get different destinations, and this is the easy thing to get backwards.** The person being corrected is reassigned to a leader in their **new** Network — they are the one whose Network is changing, and the trigger compares their replacement edge against the corrected value already in force. Each disciple is reassigned within their **own, unchanged** Network, because nothing about them has changed and their new leader must match what they still are. An implementer who applies one rule to both builds exactly the write the trigger rejects.
