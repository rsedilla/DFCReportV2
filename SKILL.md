...
**Where the configuration is absent the check fails closed, and the API still starts.** A fresh installation has to boot and run the initial import (Section 2) before either Person exists to be named, so this cannot be a value the process refuses to start without. Absent, it means no `SENIOR_PASTOR` account can be provisioned and any existing row confers nothing, and the process says so at startup. A **malformed** value — anything that is not one or two distinct, well-formed identifiers — does stop the process, because a typo that silently strips both Senior Pastors of their authority is the likeliest failure of this arrangement and the one that would be noticed last.
