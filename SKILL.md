...
**The tables and the capabilities exist; nothing else does.** Migration `0017_growth.sql` created `conquest_confirmations` and added the three `conquest.*` capabilities, and there is no endpoint, no screen, and no sidebar item whose tab it would be (Section 28). Tests are owed by each change that builds a part, and the schema's are in `api/test/database/growth.spec.ts`.
...
**The tables and the capabilities exist; nothing else does.** Migration `0017_growth.sql` created `suynl_lessons` and `training_graduations` and added the six `suynl.*` and `training.*` capabilities, and there is no endpoint and no screen. Tests are owed by each change that builds a part, and the schema's are in `api/test/database/growth.spec.ts`.
