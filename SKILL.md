...
Where the actor holds the capability by some other route — another role's defaults, or an explicit Whole Church grant — and it is the withheld **exemption** that refuses, that is a statement about the actor's authority over a target rather than about what they hold, and it answers `SCOPE_DENIED`, exactly as Section 5 invariant 4 does for every other actor. Nothing here is a special case of that rule; the account simply no longer holds a role that rule exempts.
