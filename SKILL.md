...
**Enforced by refusing the shape, not by comparing two instants.** The trigger refuses a `dcc_attendance` row naming itself as its own successor because it is one. Stated because the first version enforced it as a side effect: a self-referenced row was compared against its own `recorded_at`, which differs from its own `superseded_at` on any close of non-zero length — so a close whose two ends were the same instant compared equal and passed every constraint in the schema. The premise this paragraph rests on was then true of every self-close but one, which is the same "resting on nobody writing the row" this paragraph was rewritten to stop doing.
