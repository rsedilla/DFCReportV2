...
**This settles the leadership half of the obligation below, and not the membership half.** That paragraph puts the same obligation on Cell *membership*, which is a different case with a different remedy: a member is not a leader, membership does not mirror pastoral assignment (Section 10), so refusing while a person *leads* anything reaches none of it. Correcting an ordinary member's Network today leaves them in a Cell of the other Network by the same mechanism described here. It is recorded as open in `CLAUDE.md` rather than settled in passing, because the remedy differs — ending or moving one membership is a single authorized operation, not a handover — and a rule that reads as symmetric would be decided by the shape of this one rather than on its own terms.
...
The same applies to Cell membership: a Network change must not leave the person holding relationships that the homogeneous-network rule no longer permits. Cell **leadership** is settled above and no longer flagged; membership is not, and is recorded as open in `CLAUDE.md`. Where resolving this requires choosing between legitimately different facts, flag it for authorized human resolution rather than guessing (Section 3, Person Merge).
