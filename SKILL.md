...
**What would change this is a route that reads per-person attendance, and the answer differs by domain.** This paragraph said "No *Cell* route does" until the ruling of 2026-09-09, and **`GET /api/v1/cells/{id}/meetings/{meeting_id}/roster` now carries each member's mark**, so the 2^N bound above is gone and what it was supporting has to be re-derived. It is, here, rather than inherited — which is what the withdrawn sentence asked of the first surface to do this.

**Three grounds, and the first is why the field exists at all.** Withholding it is not neutral: Section 13 requires a `HELD` submission to carry **the whole roster**, so a correction screen that cannot read the marks renders every member unmarked and the submission overwrites every other member's mark with a blank one. The disclosure question is therefore between publishing the figures and losing them, and Section 20's reconciliation depends on an absent row and a row marked absent staying different facts.

**Second, the route admits exactly the actors the submission does.** The roster and the submit route carry an *identical* capability and target declaration — `cell.take_attendance` against the meeting, resolved through its frozen responsible leader — and `test/unit/capability-scope-resolution.spec.ts` asserts that identity, so the change that separates them reddens a test. Every actor who can read these figures may already overwrite the record they describe, which is the second ground the acceptance above already rested on and the one the 2^N bound was never carrying.
...
*What is genuinely given up is the cost, and it is given up knowingly. Before this a reader authorized to record a meeting could recover N members' marks only by 2^N submissions; now they read them. Nothing else moved: the same actors, the same write authority, and one fewer way to destroy a record by accident.*
