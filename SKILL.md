...
- **Its granularity follows the domain's unit, and the two differ.** A Cell amendment is one entry targeting the Cell: the meeting is Section 14's version unit, one meeting has one leader, and the entry is then readable exactly where the meeting is. A DCC amendment is one entry **per person recorded**, each targeting that person, because Section 14 makes the person the unit there and a submission may name people belonging to many different leaders — a single entry would resolve through one target and be invisible to every other leader whose people's figures moved. *A first implementation wrote one DCC entry targeting the amender's own Person, which put it in the amender's upline scope and out of reach of exactly the leaders it concerns.*
