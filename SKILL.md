...
**This limit closes one route to that authority and not the only one.** The role catalog above permits Admin to grant any capability explicitly, and nothing here forbids granting a withheld one to a Senior Pastor's account — which reaches the same place with no `ADMIN` row and no constraint violated, and which the identity check does not see either, since it filters role rows rather than grants. Whether such a grant may be issued is **not settled by this section** and must not be inferred from this limit; until it is ruled on, the limit should be read as what it is — the role-combination route closed, deliberately and no further.
