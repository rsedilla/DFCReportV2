...
**The report lists every root row separately from the candidate list, whenever it reports at all, because a root row that matched nobody has no entry there** — and `USE_EXISTING` is accepted for any `row_id` in the file with any well-shaped Member ID, whether or not that Person was ever a candidate for that row. A warning printed only beside a candidate would miss the hand-typed case entirely.
