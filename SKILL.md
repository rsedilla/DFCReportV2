...
**Two lock classes, and they are ordered against each other.** Ordering advisory locks among themselves is not enough once an operation takes row locks as well. A person advisory lock is taken first, in ascending lock key; **every** row lock the operation needs is taken after them, in ascending identifier, and taken **explicitly and up front** rather than left to arrive whenever a statement or a deferred trigger happens to reach for it. Both classes, in that order, before the operation decides anything.

The words *every* and *up front* are what make the rule complete, and a first version of it had neither. A deferred constraint trigger takes its own row locks at commit, in the order the rows were written, which no ordering rule can reach after the fact — so an operation writing two rows in two Cells acquires those two Cell locks in write order unless it has already taken them sorted. Two such operations, disagreeing on which Cell they reach first, cycle exactly as two unsorted callers would. Taking them up front makes the trigger's later acquisition a no-op, which is the only way the rule binds what a trigger does.

An operation states the **strength** it takes each row lock at, because strength decides whether a meeting between two operations is a wait or a cycle: two operations sharing a row at a shared strength wait for nobody, and the same pair at an exclusive strength serialize.

Without the cross-class rule the two orderings are each internally consistent and still cycle. Adding somebody to a Cell takes the advisory lock on the person and then, at commit, a row lock on that Cell; a Cell closure that took its Cell rows first and reached the people afterwards would hold what the first wants and want what the first holds. Nothing in either rule sees it, because each governs one class.

An operation that must know which people to lock before it can lock them reads that list first, outside the lock. That read may be stale, and the row lock taken afterwards is **not** sufficient to make it safe: it stops a writer still in flight, and does nothing about one that committed between the read and the lock. Such a person is in no lock set, and an operation that ignored them would meet a raw constraint violation at commit.

So the operation **re-reads the list once its row locks are held, and takes person locks for anything that appeared**. That is the one permitted acquisition of a person lock after a row lock, and it is permitted because the ordering rule guards a cycle between operations competing for their *first* locks: an operation already holding the row lock has excluded every writer that could still add to its list, so nothing can be racing it for what it is now taking. An operation relying on this says so where it is written, and the second read is part of the rule rather than an implementation detail.
...
Both halves are constraints rather than conventions, on the same terms as the leadership and membership rules beside them. Migration 0009 enforces the ACTIVE side only, and records this as an open question in a comment above the check rather than in its header; the CLOSED side arrives with the closure endpoint, in a migration of its own.
...
**The effective date has a floor, and below it no write satisfies the constraints.** It is the latest of: the start of every row **already in force** on the Cell in every effective-dated table this closure ends — membership, leadership, category and schedule — and the end of every such row already closed. A row is in force when its `started_at` is at or before the effective date.

**A row that has not started yet is closed at its own `started_at` rather than at the effective date**, which leaves the zero-length row Section 5 already makes inert, and it does not enter the floor at all.

That qualification is not decoration and the rule was briefly written without it. A schedule change takes effect at the start of the following month, so a change decided on 12 August writes a row starting 1 September and closes the outgoing one on the same instant — both in the future. A floor reading those would sit in September, and `cell_schedules_period_ordered` would refuse every date below it, so a Cell whose leader had merely rescheduled could be closed by nobody: not its leader, not Admin, because a closure dated 1 September is *forward*-dated and this section provides for no such thing. The floor exists so that an operator is given a date rather than a constraint violation, and a floor that no date can clear is the same failure wearing the other face.

The floor covers all four because the closure writes all four, and the first version of this rule named only two. Category and schedule carry the same `ended_at >= started_at` check as the other pair — all four are immediate table checks, and an earlier version of this paragraph contrasted them as though the other pair were deferred, which is both false and unrepresentable, since no `CHECK` constraint can be deferred. What makes the widening necessary has nothing to do with when a violation fires: a schedule change takes effect on the first of the following month and a category change on the day it is made, so either can be the **latest** row on a Cell whose memberships and leadership are much older — and a closure backdated past it is refused by a constraint the stated floor could not see.

A Cell cannot close before it existed, so `cells.created_at` bounds it as well. That term is a domain rule rather than a constraint: nothing in the schema compares the two columns, and it holds today only by being dominated by the first leadership row.

**The floor says how far back a closure may be dated; `records.backdate_effective_date` says who may date it back at all.** Any effective date earlier than the current day requires that capability (Section 7), which is Admin's. The closer may always date a closure today, so nothing is blocked — what they give up is the accuracy of a denominator for the days between the Cell's last meeting and the closure being recorded.

That trade is deliberate, and the reason is the denominator rather than consistency with Sections 4 and 5. A leader who has submitted nothing all month and then closes the Cell effective the first of it leaves that month with almost no scheduled meetings, and the record of their silence goes with it. Section 13 exists to obtain honest reporting, and a date field that quietly erases a month is the same defeat as a status that punishes honesty. Letting the closer reach back within the open reporting month — the window Section 13 gives attendance — was considered for its consistency and rejected for handing that vector to every leader, inside exactly the period where it does most damage.
...
**The transaction takes its locks up front, in the order Section 5 sets: the person advisory locks first, in ascending lock key, then the Cell row locks in ascending identifier** — the closing Cell and every destination — before it writes anything. A dispersal opens a membership per person, which is the same write an ordinary move takes a person lock for, so a closure needs both classes and the order between them is what Section 5 settles.

**A membership move obeys the same rule, and an earlier version of this paragraph said only a closure changed.** That was wrong twice over. A move writes two membership rows, in two Cells, so the deferred trigger takes *two* Cell locks at commit in the order the rows were written — not in ascending identifier — and a closure dispersing into the Cell a move is leaving can therefore still cycle with it. A move takes both Cell rows explicitly and sorted, up front, which makes the trigger's later acquisition a no-op. The rule is Section 5's, because it spans two lock classes and Section 5 owns the locking discipline; what belongs here is that both of this section's operations obey it.

**Strengths.** A closure takes its own Cell exclusively, since it is about to change that Cell's state, and every destination at a shared strength, since it only needs them to stay open. A move takes both of its Cells at the shared strength, for the same reason. Two dispersals into one destination therefore wait for nobody, and only an operation actually closing a Cell excludes the others — which is the least serialization that still keeps a Cell from closing underneath a membership being written into it.

The members to be locked are read before the Cell is locked, and re-read once the Cell locks are held, with person locks taken for anything that appeared in between. Section 5 gives the rule and the reason: the first read can be stale, and the row lock alone does not fix it.

Two leaders closing Cells and dispersing into each other's would otherwise take their locks in opposite orders, and that is a deadlock rather than a wait, so no timeout bounds it and the database rather than this specification picks the victim. The bound on the wait is required here as it is there, because a closure that took no lock would otherwise never set one and the row locks its own checks take would be unbounded.

**Scope over every Cell the operation touches is checked again inside that transaction, after the locks.** The guard decides on the connection pool before the transaction opens, so a handover landing in between leaves its answer describing authority the actor no longer has — the staleness Section 24 records for an intermediate ancestor, reached here through the Cell rather than through the tree. The guard's decision stays the one that refuses early and cheaply; the check after the lock is what the write actually rests on. The same applies to an ordinary membership move, whose destination is decided by the guard on the same terms — and that half is **not yet implemented**: the membership endpoint re-checks the source Cell inside its transaction and leaves the destination to the guard's earlier answer. It arrives with the closure endpoint, which builds the mechanism.
