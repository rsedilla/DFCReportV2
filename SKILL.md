...
**One thing is exempt, and it is named rather than left to interpretation: a read joined onto a query rooted in a table the reading module owns.** `hierarchy` joins `persons` in two places — `openDisciplesOf`, to name a leader's open direct disciples, and `directLeaderNameOf`, to name one leader. Both start from `pastoral_assignments`, which `people` does not own and cannot query, so the join cannot move to the owning module; returning identifiers for the caller to resolve moves it rather than removing it, and for `openDisciplesOf` turns one query into one per row. **`people` anti-joins `cell_memberships`, `cell_leaderships` and `cells` in one place** — `withoutACell`, the attention list Section 15 requires, which asks which people hold no Cell and so is rooted in `persons` (ruling of 2026-09-10). It cannot move to `cells`, which owns no table containing the people who hold none. **Shared infrastructure is a second and narrower exemption, and it is recorded rather than argued** (ruling of 2026-09-11). `src/database` is not a domain module, and `lockCellsWithin` there roots a query in `cells` to take the row lock Section 24 orders — in one place, so that every caller takes the same strength in the same order. The exemption above does not cover it, because it is a root rather than a join. It is named in `api/module-boundaries.json`, nothing else may take that shape without being named there, and **whether this section should admit it at all is open**: the read is answerable by a service interface, which is the ordinary route this section names and the route the `settings` write took the same day (decision 0237). *That said "eight days earlier" until it was checked: both rulings are dated 2026-09-11, and the interval was offered as the force of the argument.* `CLAUDE.md` carries it as a Stop Condition. *It was written into the ledger and into the ruling and not into this section, which is the "all three legs" rule broken by the change that added the check for it.* **The instances are not enumerated here** (ruling of 2026-09-11). `api/module-boundaries.json` holds every one, and `api/scripts/check-module-boundaries.mjs` derives them from the tree on every `npm run lint`, failing on a cross-module read the ledger does not name **and** on a ledger entry that matches no read. The three above are argued because their reasons are the argument for the shape; the list itself lives where something can fail on it. A total in prose is the part of a rule nothing checks, and this one was wrong three times — the third time in this very sentence, which claimed a closed list while `cells` joined `persons` in two places it did not name.

*That was a Stop Condition until 2026-09-11 and is settled by separating the two: this paragraph argues the shape, and the ledger records the instances (decision 0242). The derivation found eight cross-module reads across six methods — the three argued here, the two `cells` joins onto `persons` no home had ever named, and one in shared infrastructure that no clause had mentioned in either direction. Earlier versions of this paragraph asserted a total twice more, "two modules qualify, in three places" and that it had been "widened once", and both were false of the tree. No total is stated here now, because a total is exactly what a paragraph cannot keep true.*

*A first version of this paragraph said the joins were in `hierarchy`'s recursive walks and that asking `people` per row would “turn one query into hundreds”. Neither is true of the two `hierarchy` joins it exempts: `ancestorsOf` and `subtreeOf` select identifiers only and join nothing, and `directLeaderNameOf` returns at most one row. The category was right and the description was written from what the exemption was for rather than from the queries — in the paragraph added to stop exactly that.*

**`reporting` is the module this rule bites hardest, and it takes the ordinary route** (ruling of 2026-09-05). It owns `report_snapshots` and `notifications` and nothing else, so it roots no query in attendance, Cell, membership, Person or pastoral-assignment tables, and a whole-church monthly report is therefore not a join. The owning module computes its own aggregates — `hierarchy` the dated subtree walk, `attendance` the DCC and Cell attendance figures, **`cells` the Cell coverage denominator**, whose every input (`cell_schedules`, `cells`, `cell_leaderships`) it owns, and **`people` identity, lifecycle filtering and the drill-down population** (§16 excludes archived and merged Persons from Participation, which needs `person_lifecycle` and `merged_into_id`) — and `reporting` composes them and stores the snapshot. *A first version named two computing modules and directed `attendance` at three tables `cells` owns — the rule this paragraph exists to uphold, broken inside it. The correction then named three while the count in its own next sentence said four, and the missing one was `people`.* Widening the exemption **for `reporting`** was considered and refused: it would reach around eleven tables across four modules, which is not an exemption but this rule reversed for one module.
...
**A standalone lookup in another module's table is the main rule's own case, not the exemption's** (ruling of 2026-09-11). The exemption above is about a read *joined onto a query you root yourself*; a lookup by identifier is joined onto nothing and is precisely what "for anything a service interface can answer" names. Four reads existed and were re-homed rather than admitted, and they were not alike. **Three were standalone lookups** — a name by identifier, a Person by account — and each became an ordinary service call. **The fourth was not a lookup**: a paging query *rooted* in another module's table, which split into a service call for the edge set it is rooted in and a port for the one read whose direction is a cycle. So four calls and one port, chosen by whether the direction was a cycle and by nothing else. *A first version of this paragraph called all four “such reads” and counted three calls, which put the paging query in a category it does not belong to and dropped the `hierarchy` service call from the count entirely.*
...
JSON encoding rather than a delimiter, because a name may legitimately contain any character (Section 3) and no delimiter is safe to assume absent. *Two characters are refused at the edge under Section 22 — a null byte and an unpaired surrogate — and that narrows what a field may hold rather than giving this fingerprint a delimiter it could rely on: neither is a character anyone would separate fields with, and the import reads a file rather than a request.* Trimmed values rather than raw, because surrounding whitespace is precisely what a spreadsheet adds and removes on its own — the class of change this fingerprint exists not to refuse.
...
It is guarded by `people.view_subtree` against the actor, with the scope narrowed in the owning module, and it **names no period**: somebody reassigned last week needs no action today, so it asks about now and is an undated viewing read under Section 7. It is ordered by name, is never ranked, and carries no measure of how long a gap has stood (Sections 13, 15 and 17) — a list ordered by staleness is a ranking of neglect whatever it is called. Each entry carries the reassignment that resolves it, which this section governs; appearing on a list confers nothing toward performing it.
...
- **A meeting's roster is guarded by the capability that records it**, `cell.take_attendance`, resolved against the meeting, and it carries each member's recorded mark to every actor it admits (ruling of 2026-09-15). `cell.correct_subtree` guards changing a record that already stands and nothing about reading it. `GET /api/v1/cells/{id}/meetings/{meeting_id}/roster` answers who there is to record, which is what taking attendance needs, and it is the exact counterpart of `GET /api/v1/dcc/events/{id}/roster` one domain over. It is deliberately **not** `cell.manage_membership`: that capability governs changing who belongs to a Cell, it is a management capability so `read_only` is invalid on it, and requiring it to record a meeting would mean nobody could take attendance without also being able to move the roster. The same argument moved `GET /api/v1/cells/{id}/members` to `cell.view_subtree` on 2026-09-04 (ruling of that date): guarded by the management capability, roster visibility could not be granted at all, because a management capability granted `read_only` is rejected at creation. *This sentence read "which manages membership and stays where it is" while the reason to move it was believed unavailable — Section 7's list was read as closed against a capability already in it.*
...
- a **Cell**, a Cell meeting, a membership or a leadership resolves through the Cell's leader **as of the period being viewed**, falling back to its last leader where the Cell is closed. A closed Cell keeps its history and its roster visible to the leader who led it (Sections 10 and 15), which resolving through a current leader it no longer has would prevent. **A person's current Cell, read from the person, is not this target** (ruling of 2026-09-15): it resolves through the person rather than through the Cell's leader (Sections 8 and 10)
  ...
  - **The clause above governs an operation whose target is the closed Cell, and a second object a request names is not one** (ruling of 2026-09-21). Where a request's target is a Person and it names a Cell besides — Section 10's second-object rule, which a handover and a restart both carry — the scope check on that Cell is a read of who led it, and it resolves by the base bullet above, falling back to the last leader where the Cell is closed. So a restart is requested by a leader upline of the person who led that Cell (Section 10). **It is a narrowing of the clause rather than a reading of it**: the clause reasons from the capability's class, and this says the operation's class is what it is about. **What the operation's target is, is the whole of the test** — not whether the Cell changes, which would reach further than this rule does. `GET /api/v1/cells/{id}/meetings` has the closed Cell as its own target, so the question the bullet above records stays open for it, in both its halves: the admission, and whether that admission survives the month's window shutting
...
- a **report scope selector** is itself the target: a request for a scope the actor does not hold is `SCOPE_DENIED`, never silently narrowed to what they do hold
  - **A `NETWORK` selector is covered by a Whole Church grant, or by a `NETWORK` grant naming that Network, and by no subtree grant at all** (ruling of 2026-09-07). That last clause is not a separate rule and does not depend on anybody actually being unplaced: a `NETWORK` selector names **no Person**, so there is no containment for a subtree grant to be tested against, and Section 20 makes a Network's population its *membership* rather than any subtree. *An earlier version argued it from the residual — that a root's subtree "still excludes" one — which is a fact about the rows on a given day: with nobody unplaced a root's subtree and the Network's membership coincide, and the argument would evaporate while the rule stayed right. Admitting a subtree grant would then make an authorization answer depend on whether somebody had been encoded and not yet placed, which is the reasoning this section refuses everywhere else.* Nobody is affected in practice: both Senior Pastors hold Whole Church (Section 4), which is what makes the two Network selectors Section 17 offers them work
  - **It resolves as of the period being reported** (ruling of 2026-09-05), which is the capability rule above applied to the one target that had no instant — `reports.view_subtree` is named there and the selector's instant was not. An open period resolves at the same instant a closed one does — the period's final millisecond (ruling of 2026-09-07). *This section said "as of now" until then, repeating the half of Section 20 that its own neighbouring sentence contradicted.* So a leader may read October's figures for a downline leader who left their subtree in November, and may drill into them: the total is computed from the October tree and already contains that leader's people, so authorizing undated would show a figure while refusing the breakdown that explains it, against Section 20's additivity — on the actor's own screen rather than in the arithmetic, since the identity still holds and what breaks is the actor's ability to see one of its addends. Historical visibility follows historical responsibility, which is this section's own reason, given in the base bullet above where a Cell target "resolves through the Cell's leader **as of the period being viewed**" and cited there to Sections 10 and 15. *A first version cited those two sections as giving the reason. They do not: §10 says a closed Cell's recorded attendance and prior reports are unaffected and gives no visibility rule, and §15's only closed-Cell provision is the attention list, which is bounded by the submission window and justified by a recording obligation rather than by historical visibility. It also cited the closed-Cell **fallback**, which is the undated half of that bullet, where the dated half is what carries this.*
  - **It resolves through the pastoral tree in force at that instant, and never through Section 20's placement graph** (ruling of 2026-09-06). The two are different graphs and the placement graph is strictly wider: it exists so that a report's totals sum, placing a person with no open assignment at the period's end by the last one they held, and continuing the chain past a leader who left. Section 20 requires the condition that widens it to be **surfaced on an attention list** as a gap needing repair, so reading the same fact as a grant of visibility would turn a defect into a licence — a leader could read the figures of somebody whose only connection to them is a chain through a person who left years earlier, which Principle 5 refuses. **The wider graph computes and the narrower graph authorizes.** What that costs is accepted and stated: an aggregate may cover people the actor could not reach one at a time, which is tolerable exactly while the report discloses no identity. A per-person view is a different question and is not settled by this.
  - **A `CELL` selector resolves through the Cell's leader in force at that instant, and where the Cell holds none there, through its last leader** (ruling of 2026-09-07). The base Cell bullet above is the only sentence in this section that says what a Cell resolves *through*; this bullet has only ever supplied an instant, because `LEADER` names its person and `NETWORK` names none. So the two are read together rather than chosen between, and the closed-Cell fallback reaches this target as it reaches every other Cell target. **The fallback fires only where the instant finds nobody** — after a closure, in the period a closure falls in, and before the Cell existed — so a **handover** never invokes it: each past period resolves to whoever held the Cell at that period's end. **That answer was confirmed deliberately rather than left incidental** (ruling of 2026-09-10), against the argument that Section 13 and decision 0187 settle the structurally identical question one unit down — which leader a meeting on the handover day belongs to — as the **outgoing** one. The two do not conflict, because they are asked at different granularities: a meeting has a date of its own, and 0187’s reason is that attribution must not depend on when the record was entered, while a period has no date of its own but two ends. Resolving it at its final instant is a choice of end rather than a choice of clerk, so that reason does not reach it, and carrying it upward would resolve a period through whoever held the Cell when it *began* — the mirror-image loss rather than an improvement. The period a closure falls in is what makes this worth stating: Section 12 says a Cell "closed part-way through a month has fewer" scheduled meetings "and that is not an anomaly", so that month is reportable and holds real recorded attendance, and resolving through nobody would withhold it from the leader who recorded every row of it. **A `CELL` selector names a Person once resolved, so a subtree grant covers it** where that leader is within the actor's subtree at the same instant, and a `NETWORK` grant covers it where that leader's Network at that instant is the granted one — the `NETWORK` selector's "no subtree grant" clause above rests on naming no Person at all and does not reach this. A Cell that does not exist resolves through nobody, so **every grant narrower than Whole Church** refuses it exactly as it refuses an out-of-scope Cell. A **Whole Church** grant covers a selector before the target is read at all, so it reaches the existence check rather than this bullet's resolution — which was never this bullet's rule failing but the open question about a selector naming somebody who does not exist, arriving at a second target kind. **It is settled by the *selector naming nobody* bullet below** (ruling of 2026-09-17), which binds this target kind on the same terms: a Cell that does not exist answers `NOT_FOUND` to a Whole Church grant and the same `SCOPE_DENIED` as before to every narrower one
  - **A selector whose identifier names no Person or Cell answers `NOT_FOUND`, and scope is resolved before existence is consulted** (ruling of 2026-09-17). *Not the `NETWORK` selector, which names no Person by design and is governed by its own bullet above; this is about an identifier that refers to nobody.* A report is a figure about a subject, and where there is no subject there is no figure: a well-formed identifier that names no Person — `NIL_UUID` included, which this application hands over everywhere else precisely so that a request refuses — answers `NOT_FOUND`. **The ordering is the rule and not an implementation detail.** A grant narrower than the selector keeps receiving the refusal it receives today whether or not the identifier names anybody, so that each actor gets one consistent answer and which one they get is decided by their own scope rather than by the record — which is Section 22's rule for a Cell, applied to the one target kind it had not been read against. *A draft grounded this on closing an existence oracle, which Section 22 refutes for this target kind in terms: "People are not such a case: Section 8 already discloses minimal identity church-wide by design."* It binds a `CELL` selector on the same terms, because one silence in two places is one rule. `GET /api/v1/people/{id}/pastoral-path` already works this way and says why in its own comment; this states it as the rule the next such route follows. *`VALIDATION_FAILED` was refused: the request is well formed and names nobody, which is a different thing from malformed, and Section 22 keeps that code for a request's shape rather than for what it refers to.*
  - **A `NETWORK` grant resolves through `network_as_of` at the instant the target carries** (ruling of 2026-09-07). For this target that instant is the one the selector itself resolves at, so a grant naming the Men's Network covers a leader who was in it **at that instant**, whoever they are today. Not "at any point during the period": a leader who moved Network mid-period is covered by the grant naming the Network they held at the instant, and not by both. That is the same single-instant rule the subtree walk beside it uses, and it holds whichever instant that is — which is the period's final millisecond whether the period is open or closed (ruling of 2026-09-07). *Until that ruling this clause said there was no settled answer to inherit for an open period, because Section 20 stated two — and it was right to decline: the Network axis follows the subtree axis to whatever instant it resolves at, and nothing about Networks turned on which.* Section 4 states the reason the history exists in these terms — "every Network-scoped report for a closed period depends on that answer" — and resolving the subtree dated while resolving the Network undated would have made the two axes disagree about one request. A Network unknown at that instant covers nothing: `network_as_of` answers null before the person was encoded, Section 4 is deliberate that nothing is inferred from before then, and the comparison therefore refuses rather than guesses. *This replaces an earlier ruling of the same day, which refused such a grant outright on the ground that no dated Network resolution existed. One did — `network_as_of`, already called from three modules — and that ruling's own text said so while this section said the opposite.*
  - **The walk runs outside the report's own transaction** (ruling of 2026-09-06). Authorization is decided before the reporting transaction opens, on the pooled connection every other guard in this system uses, which is what keeps the third clause of Section 24's isolation exception true. Deciding scope from the snapshot the figures are computed in would be that section's *lock-then-decide* staleness with the lock removed. The cost is that the guard and the figures read at two instants, and it is stated in both directions. For a closed month both resolve against fixed history and cannot differ except under a **concurrent backdate**, which is left as a residual — Section 20's invalidation list governs when a *stored* figure is recomputed and does not make two live reads agree. For an open period a person moving **under** the selector after the guard ran was already within the actor's reach through that selector, while the selector itself moving **out** of the actor's subtree returns figures to an actor no longer authorized over it, bounded by one request. Neither is a statement about what a subtree scope means generally; both are properties of deciding before the transaction opens.
...
**That rule binds a submission that lost a race, and it binds both domains** (ruling of 2026-09-04). A first submission whose writer lost to another and whose contents **differ** from what was committed is answered a conflict carrying the stored figure and the name of the account that recorded it. *This read "which neither roster publishes to an actor holding only the recording capability" until 2026-09-09; the Cell roster now publishes the marks that figure is derived from, and the DCC roster always published its own. What keeps the ordering right is the rule above rather than a claim about which routes exist.* So the amendment capability is decided before that conflict is built, on the disagreeing branch, in the Cell domain and the DCC one alike. **On that branch only**: a loser carrying what the winner recorded wrote nothing, is answered `RESOURCE_BUSY`, and owes no amendment capability by the rule above — checking it there would refuse `403` what the identical body answers `201` sequentially, and Section 22 stores a 4xx against the idempotency key, so a conforming retry would replay that refusal for ever.

**The amendment capability and nothing else.** Every other capability the write owes is decided before the record's **contents** can change what the caller is told, and must be: an on-behalf capability governs whether the record is the actor's to touch at all, which is not a question about its contents.

*Stated over what the caller is told rather than over what the code reads, and that is the rule rather than a gloss on it.* What must not happen is a refusal, a success, or a status code that differs according to the contents before the capability is decided. Reading them earlier is permitted, because the amendment capability is decided **by** the contents and they have to be in hand for it.

*The two paths differ here, and the difference is why the rule is stated this way.* The **Cell** path reads no attendance until after the on-behalf check — it obeys the older, stronger form of this sentence and still does, which the paragraph below says in terms. The **DCC** path loads the event's live records in one query before the per-line loop, so the read-order form was never expressible for it without forbidding a batch read that discloses nothing. *A version of this sentence claimed no implementation obeyed the stronger form. That was true of DCC and false of Cell, and the next paragraph refuted it four lines later — the fourth restatement of one sentence in four commits, and the third to be introduced by the fix for the last.* Ordered the other way the *success* answers what the refusal was withheld to protect, which is a defect this specification has now been given twice — and it is why `dcc.submit_on_behalf` is required of an unchanged line while `dcc.correct_subtree` is not.
...
**Both implementations obey it, and the DCC one did not until 2026-09-03** (ruling of that date). `cell-meetings.service.ts` refuses the status change, then decides `cell.submit_on_behalf`, then reads `cell_attendance.present` — contents last, as stated. `DccAttendanceService.writeWithin` derived a line's outcome from the stored `present` and refused a `CREATE` carrying a `correction_reason` **before** `assertMayRecord` decided `dcc.submit_on_behalf`, so one request answered `409` where the person had no live record and `403` where they had one. The refusal now runs behind the on-behalf check and the pair is gone.

*Deriving the outcome beforehand is not a violation, for the reason the rule itself now gives: nothing the caller is told depends on it until the capability has been decided.* The alternative considered was writing DCC an exception into this section, and it was rejected — nothing distinguishes the domains here, since the question the ordering protects is answered by whether the person is on the actor's checklist, which depends on nothing stored about the record. An exception would have rested the guarantee on which grants happen to be issued, which is the thing the ordering exists not to do.

*Two earlier versions of this paragraph were wrong in opposite directions, and both are recorded because the second was written while correcting the first: one claimed every implementation obeyed the rule when DCC did not, and its correction described the violation as an open question about safety rather than as a violation.*

**The Cell path is safe on a ground that is checked rather than assumed.** What its early refusals actually expose is the meeting's **status** — `current_status` on the status-change refusal, and the bare existence of a record on the others. The stored `version` reaches an actor through the no-op success, which is *after* the on-behalf check, and through the `RESOURCE_BUSY` a lost race answers, which carries `current_version` and is deliberately not gated on the amendment capability — an agreeing submission writes nothing and owes none (Section 22). `submitted_by` is never returned by the write at all. *A version of this sentence said "only through the no-op success", which the lost-race answer made false the day it was written; both paths are named now, and both are covered by the roster read below.* `GET /cells/{id}/meetings/{meeting_id}/roster` hands the same actor the status, the version and the submitter under an *identical* capability and target declaration — so it covers what the refusals expose with room to spare, and the margin is stated rather than left as a claim that the two sets are equal. If that read is ever given a viewing capability of its own, the two routes stop admitting the same actors and this has to be re-derived rather than inherited. `test/unit/capability-scope-resolution.spec.ts` asserts the two declarations are identical, so the change that separates them reddens a test instead of quietly widening what a refusal answers.

**That is a Cell argument and does not transfer, which is why DCC needed its own.** A version of this paragraph asserted the Cell ground over both paths on 2026-09-03 and was wrong to: a DCC event resolves through the people a route returns rather than through its declaration (above), so the identical declaration on the DCC roster and submit routes establishes nothing about which people each reaches. The roster walks the actor's checklist; the submission reaches everyone they hold `dcc.take_attendance` over. What makes the DCC ordering safe is the ruling above — the on-behalf check runs first and refuses identically whatever is stored — and not any property of its declarations.

**For a Cell meeting** it is accepted rather than closed, and the bound **was** why — was, because the paragraph below retracts it and says what the acceptance rests on instead. The submission must name every member of the meeting exactly once (Section 13) and the answer covers the whole set with no per-line detail; **while nothing else published the marks**, it followed that recovering N people's attendance **cost** 2^N submissions rather than N. *Two premises, and the one that died is the second. The submission's shape is the first and is unchanged, which is why the conclusion is past-tensed on the loss of the other rather than on any change to the route — and two earlier attempts each drew it from premises that do not yield it: one scoped the conclusion to the submit route and past-tensed it together, where through that route the cost is what it always was, and one past-tensed the unqualified conclusion while stating the shape alone.* *The domain is named because that shape is a fact about a Cell submission and never held for DCC, which the bullets below give its own ground; an unqualified "it" here asserted the bound generally and is withdrawn by the DCC bullet below that names the domain.* And every actor who reaches it holds the capability that records this record and may overwrite it outright by holding one more — which is a ground in its own right, one of the two decision 0191 has left, and is untouched by the bound going.

**What would change this is a route that reads per-person attendance, and the answer differs by domain.** This paragraph said "No *Cell* route does" until the ruling of 2026-09-09, and **`GET /api/v1/cells/{id}/meetings/{meeting_id}/roster` now carries each member's mark**, so the 2^N bound above is gone. **Two acceptances had to be told apart once it went, and only one of them needed new grounds.** The bit the paragraph above accepts — whether the record you sent is the record that is stored — **survives** rather than being re-derived: decision 0191 gave it three grounds, the bound was one, and the other two stand untouched and pre-date the withdrawal. Neither is a cost argument: the actor is not a stranger to the record, and refusing an unchanged submission tells a leader they may not alter what they did not alter. **Publishing the marks is a second acceptance, and it is the one re-derived here**, on the three grounds below, rather than inherited — which is what the withdrawn sentence asked of the first surface to do this. *Reading the two as one acceptance is what produced a summary sentence that was written three times and refuted three times before being deleted: the three grounds below are the roster's and were never the bit's.* *The meeting resolves through its **current** leader on an `ACTIVE` Cell and through the frozen responsible leader only on a closed one whose window is open, as the bullets above state; a first version of this re-derivation named the frozen leader unconditionally.*

**The first ground is why the field exists at all, and it is not a disclosure argument.** Withholding the marks is destructive rather than neutral: this specification requires a `HELD` submission to carry **the whole roster** (Section 13), so a correction screen that cannot read them renders every member unmarked and the submission overwrites every other member's mark with a blank one. That is a leader correcting one person and silently erasing the rest.

**The second ground is that the route admits exactly the actors the submission does, and it stops there.** The roster and the submit route carry an *identical* capability and target declaration — `cell.take_attendance` against the meeting — and `test/unit/capability-scope-resolution.spec.ts` asserts that identity, so the change separating them reddens a test. **It does not follow that every actor who may read these marks may change them**, and the unqualified claim was written here once and is withdrawn: correcting a record that already stands needs `cell.correct_subtree`, so an actor holding only `cell.take_attendance` now reads a figure they may not write. This section had already made and withdrawn exactly that overstatement for DCC, four paragraphs down, and the Cell paragraph reintroduced it.

**Third, it publishes no per-person `version`**, which is where the DCC answer and this one part company. Section 14 makes a DCC record's unit `(dcc_event_id, person_id)`, so that domain *must* publish a per-person version or no client can correct anything; a Cell submission carries the **meeting's** version and `cell_attendance.version` is never compared (Section 14; rulings of 2026-08-31 and 2026-09-03). A per-person version here would be a number the client must not send back, offered beside the fields it must.

**What is given up, exactly.** Before this, an actor authorized to record a meeting could recover N members' marks only by 2^N submissions; now they read them, and that includes the actor who may not change them. The route reaches no new person — the declaration is unchanged — so what widened is what a reader already admitted may learn.

**Which capability reads the marks is settled: `cell.take_attendance`** (ruling of 2026-09-15). The capability-boundaries list above once named `cell.correct_subtree` for the roster as well, which left this section disagreeing with itself and with the route; it now names that capability for the correction alone. Carrying `record` only for an actor holding `cell.correct_subtree` was set out and not chosen: it would change what the API gives an actor the route already admits, for a case no role default produces, and it would present a recorded meeting as names with nothing against them. It is decision 0194's reason for the DCC roster, applied to this one.

**For DCC the figures are published deliberately, and that is not a disclosure to be closed** (ruling of 2026-09-03). `GET /api/v1/dcc/events/{id}/roster` returns each person's `present`, `version` and `recorded_at`, under `dcc.take_attendance` — a recording capability, and one this specification does not permit to be granted `read_only`. Three grounds, in the order of how much they rest on:

- **The `version` is not discretionary.** Section 14 requires a client to submit the version it read, and makes a DCC record's unit `(dcc_event_id, person_id)`. A per-person surface must therefore publish a per-person version, or no client can submit a correction at all.
- **The bound the Cell case rested on does not exist here, and no longer exists there either.** DCC is recorded and answered per line, so an actor could recover N people's figures in N probes rather than 2^N — the acceptance above would be worth very little in this domain, whether or not the roster published anything. Publishing is not what makes the figures reachable.
- **The figures are mostly the actor's own to write.** Section 9 makes a person's attendance their direct pastoral leader's obligation and says a submission is one leader's whole checklist, "so most of its lines repeat what is already recorded" — a leader who cannot see what is marked re-asks people already counted. *That a checklist is therefore prefilled is **inferred** from those two and stated by neither, and it is marked because the other two grounds are **checkable** and this one is a judgement about how leaders work. Ground one is decidable against Section 14, ground two against the shape of the route. A version of this aside said the other two were "quoted", which is the reverse of the truth: neither of them quotes anything, and this bullet is the only one that does.* *Mostly, not wholly: where a record exists and disagrees, changing it needs `dcc.correct_subtree`, so there is a figure such an actor may read and may not write. An earlier version of this sentence said "withholding it would protect a figure the same actor may write" without that qualification, which is true only of a record that does not yet exist.*

DCC is recorded per line with per-line outcomes, so no such bound ever existed there, and none is needed: the version is required to be published at all, and the figures are reachable in N probes whether or not the roster shows them. *This clause said "which none does today", of the whole system, and was false when written; a version that noticed left the consequence open and is now settled.* *A sentence stood here summarising which grounds the acceptance rests on. It was written three times and refuted every time — for asserting the dead bound in the present tense, for counting one enumeration while naming another, for naming one of the bit's two paths, and for placing the DCC grounds below itself when they are above — so it is deleted rather than written a fourth time. **The refutations are named rather than tallied against the versions**: an earlier form of this aside gave three grounds for three writings, and they did not correspond, one version having supplied two and one none. Each set of grounds is stated where it is argued: the roster's three above, the bit's two in decision 0191, and DCC's three in the bullets immediately above this paragraph.*
...
**A collection is a target too, and one whose period dates its figures resolves its membership as of that period** (ruling of 2026-09-10). The trigger above is what the request *names*, not what shape the answer takes, so a route returning a list of Cells under `cell.view_subtree` and taking a required `month` is a dated viewing read exactly as a single-target read of the same month would be. Its membership resolves at **the period's final millisecond** — decision 0218's instant, adopted rather than a second one derived — so a Cell handed over after the period ended lists for the leader who oversaw the period, and not for the one who holds it today. Two things this does not do, both stated because each was assumed once. It does **not** give a mid-period handover to both leaders: that period resolves through whoever held the Cell when it ended, which is the same answer the base bullet gives a `CELL` report selector and the same one the current-state rule gave. And it dates **nothing beneath it**. Every detail route under `/cells/{id}` resolves through the Cell’s leader **now**, and each does so under a different clause of this heading rather than as a class: `GET /api/v1/cells/{id}/meetings` and the two meeting routes beside it carry a **recording** capability, which the rule above leaves undated whether the route reads or writes; `GET /api/v1/cells/{id}/members` carries this same **viewing** capability and is undated because it names no period; and the five `cell.manage_*` routes are undated as "everything else is acted on now". **So a dated collection can offer a row whose every detail route refuses the actor**, and the members route refuses under the very capability the index is granted on — which is why "an index and the route a row links to may resolve differently" is not a consolation here and is not offered as one. What that costs is a question of its own, recorded in `CLAUDE.md`, and it is not settled here. *The meeting routes are undated only while the Cell is `ACTIVE`; the closed-Cell exception above dates them to the meeting’s own date. A collection listing `ACTIVE` Cells cannot reach that, and the clause is named so the next reader does not take "undated" for a property of those routes.*

**A `NETWORK` grant follows that same instant, and this binds all three dated capabilities rather than the report alone** (ruling of 2026-09-07). A grant at Network scope covers a person whose Network was the granted one *at the instant the request is asking about*, read with `network_as_of`. Where the request names a period that is the period's instant; where it names none it is now, and `current_network` is that same read taken at now rather than a second rule. So `cell.view_subtree`'s undated route keeps the current Network, correctly, and the next dated route inherits the **rule** rather than having to rediscover the question. It does not inherit a mechanism: the dated read exists inside the report scope selector's own resolution, and every other target kind reaches an undated branch with no instant in reach, so a dated route at either of the other two capabilities has to supply one. A Cell-targeted dated viewing route is told so by `capability-scope-resolution.spec.ts`; an `audit.view` one is told by nothing, and owes that test a case when it arrives. *This section said earlier the same day that a Network grant of a dated capability covered no record, because no dated resolution was thought to exist. One did, and neither ruling had reached `main`.*
...
**This list is enforced by the database as well as by this section**, and a capability added here owes **two** migrations before any grant of it can exist: `capability`, the enumerated type `capability_grants.capability` stores, must gain the identifier — without which no grant of it can be stored at all, whatever the flag says — and `capability_grants_read_only_is_a_read`, which enumerates the read capabilities by name, must gain it if it is one. The nine Conquest, SUYNL and Training capabilities took both across `0017_growth_capabilities.sql` and `0018_growth.sql` (Sections 27 and 28), two migrations rather than one because PostgreSQL refuses to use a value added to an enum inside the transaction that added it: the enum gained all nine, and the CHECK constraint gained the three `view_subtree` ones alone, the six confirming capabilities staying out of it because adding a write capability there would make a `read_only` grant of it storable. **The read list has a third home and it is in the application**, where a `read_only` grant of anything else is dropped at runtime; a capability added to the list above owes an edit in all three places, and naming two of them is how one gets missed — which is what happened to the three above, whose CHECK constraint was omitted from the first version of that migration and caught in review. **The three homes agree**, and `api/test/database/schema.spec.ts` compares the constraint against the application's list so that a fourth capability cannot diverge them silently. The role defaults in the catalog above are a separate mechanism and neither migration gates them: they are declared in code, and what makes a Conquest, SUYNL or Training default unusable today is simply that no module and no route exists yet.
...
**A refusal about scope is decided before anything about the record can change what the caller is told**, which is Section 7's ordering rule in this domain and is stated in its words. Whether an actor may reach a person at all is `dcc.take_attendance`, checked against the person and never against what is stored; `dcc.submit_on_behalf` is checked next, against whether the person is on the actor's checklist and so against nothing stored either; and the amendment capability is checked after both, by an actor already in scope. *This said "before anything about the record is read" until 2026-09-03 and named only two of the three, which left the strongest reading of it violated by the very path it governs — a line is loaded and compared before any of them. Section 7 was amended and this was not, in the same commit, so the two sections stated one rule at two strengths for a day.* Deciding it the other way round makes the refusal itself disclose the record: the capability named would depend on whether a record exists and on what it says, and Section 8 withholds DCC attendance for anybody outside the viewer's scope. The same order governs the refusals that describe the Person — archival, a merge, and whether they had a pastoral leader on the date are each withheld by Section 8, and only "no such person" is safe first, because Section 8 publishes minimal identity church-wide.
...
**The denominator is the month's whole schedule, including days it has not yet reached** (ruling of 2026-09-11). So a line read on the 10th shows every meeting the month holds and not only those already due, and a leader who has recorded everything so far reads `1 of 4` rather than `1 of 1`. That is deliberate on two grounds: it is what a leader can predict from their own calendar, where a bounded count changes every week; and Section 17 requires the open flag beside it, so a partial line says in terms that the month is still running. **It does not match the DCC coverage line and is not meant to** — that figure counts obligations, which accrue as each event happens (Section 9), while this one counts recorded meetings against a schedule that is known in advance.

**Behind is the meetings that have come and have no record, and it is the only thing a control keys on** (ruling of 2026-09-21). A Cell is behind by the number of its scheduled meetings whose Manila day has begun and which have no record, each matched to a record by its scheduled date — the predicate and the counting Section 17 gives the Network screen, over the schedule derived as the denominator above is and bounded at today, so a closed Cell's stop at its closure. The Cells list carries it on every row, computed by the server, and "behind" means that on every surface using the word: the Network screen, Section 15's attention list, and the Reports coverage table, whose filter shows only the Cells behind. No control keys on the whole-month denominator, which would name a Cell for meetings that have not happened, and nothing derives behind by subtracting one figure from another. It never divides the coverage line, which stays two figures; how many Cells of a list are behind is a count of that list's rows over every page, shown in words beside its complement, and the list keeps its order.
...
**Step one — the request.** A leader upline of the prospective Cell Leader submits a request naming that person. For a new Cell it also carries the category, the day and the time; for a handover it names the existing Cell instead.

**A third thing a new-Cell request may name is a closed Cell it restarts** (ruling of 2026-09-20). A Cell closes and its people sometimes come back; a closure is never reversed (Cell lifecycle, below), so resuming one is a new Cell that records where it came from. The request carries `restart_of_cell_id`, approval sets `restarted_from_cell_id` on the Cell it mints, and nothing about the closed Cell changes. Four rules bound it. A closed Cell is restarted at most once, because two Cells claiming to resume one Cell claim one history between them; a restart may itself be restarted, so the chain runs backwards and never branches. A Cell closed `CREATED_IN_ERROR` may not be restarted, that reason being a statement that it should never have existed. A restart names the Cell's **last leader**, because what the link asserts is that this Cell resumes under the person who led it — somebody else taking those people on is an ordinary new Cell, and its category, day and time are the request's own, filled in from the closed Cell and changeable. **The closed Cell carries the same second-object rule a handover's does**: the actor must have it within their authorized scope, on the terms that govern closing it, which resolves `cell.manage_lifecycle` against its leader — its **last** leader, the closed-Cell fallback reaching a second object a request names (Section 7, ruling of 2026-09-21). A restart therefore inherits the consequence stated below for a handover — an actor granted `cell.request_leadership` and not `cell.manage_lifecycle` cannot request one. The former leader may not restart their own Cell, which is the self-exclusion below rather than a new rule. And the request carries no member list: the former members are re-added to the new Cell through the membership route, which is where a membership's refusals and its audit entry already live, and a list frozen while a request waits would re-add somebody who has since joined another Cell. The capability is `cell.request_leadership` (Section 7), held over the actor's subtree **excluding themselves**. In practice this is a leader saying that one of their own disciples is ready to lead.
...
**A facilitator must be a Person who exists, and that is refused rather than left to a foreign key** (ruling of 2026-09-04). It is the only field on this route carrying an identifier a caller chooses and nothing else resolves, so a well-formed body naming an absent Person reached the constraint and answered `INTERNAL_ERROR` — Section 22's named failure mode.

**Any current Person may be the facilitator** (ruling of 2026-09-22, decision 0274): one who exists and is neither archived nor absorbed by a merge, which is what Section 10 asks of a Cell member. Nothing narrower, because the examples below are a disciple or an upline leader, and a Cell legitimately holds people outside its leader's tree. The recording screen asks "Who ran the meeting?" on a first record of a meeting that met, defaulting to the leader.
...
**A meeting that took place is recorded for every member of the Cell on its roster date,
exactly once, present or not.** `HELD` and `RESCHEDULED` alike — the reschedule takes the
roster of the day it moved to, and the rule is what makes that a complete account rather
than a partial one. `NOT_HELD` carries no attendance at all, so it is outside this. This is the rule Section 7 and this section have each cited as
"every member exactly once (Section 13)" while it was stated nowhere in its own words —
visible once the ruling below stopped resting on a Section 20 sentence that does not exist.
It generalises the zero-attendance case above rather than being deduced from it, and that
is said plainly because this paragraph exists to stop the rule resting on something that
does not say it: a meeting the leader attended alone "counts in the denominator, and every member is recorded as not
having attended", so a member left out is not a member marked absent but a member the
leader said nothing about. Nothing distinguishes the two in any figure this system computes
— both bucket views count attendees — which is why the rule is about what a leader
**declares** and is enforced at the submission rather than derived from a total.
...
not, on the rule stated in its own right above and on Section 14's "a correction is an
account of the whole meeting, sent as a roster" — so a correction resubmits marks a client had no way to read: the roster named the people
...
with blanks. Section 7 grounds the field on the three grounds it re-derives there, rather
than on the accepted disclosure it states for the confirmation bit — the two are different
acceptances and the bit's grounds were never this field's — and the route's
declaration is unchanged, so it reaches no new **person**. *This read "Section 7's accepted
disclosure reaches it", which is the inheritance Section 7 now refuses in terms.* *It was written here as
"resolved against the meeting's **frozen** responsible leader, so it adds no reader", and
both halves were corrected on review the same day: Section 7 resolves an `ACTIVE` Cell's
meeting through its **current** leader, the frozen one answering only for a closed Cell
whose window is open; and "adds no reader" is true of the route and was read as more — an
actor holding `cell.take_attendance` without `cell.correct_subtree` now reads a mark they
may not change.* They are read under `cell.take_attendance`, the route's own
declaration (ruling of 2026-09-15). It is decision 0194's
...
**A meeting whose Manila day has not begun takes no record** (ruling of 2026-09-11). The window above states when a month shuts and not when it opens, so a scheduled date later this month sits inside an open window — and a record against it would be a `HELD` meeting nobody has attended, advancing Section 12's unique-people total and a lifetime classification in a period nobody has reported. This is Section 9's rule for a DCC event, in the domain that had no counterpart, and it is the day's **beginning** rather than its end: a leader filing on the night of the meeting is who this section is written for.
...
The two rules that force this are each clear alone. A first submission cannot carry `RESCHEDULED` (above), so a meeting is recorded and *then* moved; and a rescheduled meeting's roster comes from the actual date (Section 12), because that is who could have been there. Performed as two operations, the ordinary flow records attendance against the scheduled-date roster and then moves the roster out from under it — leaving a member who joined between the dates with no record, and one who left with a record for a meeting they were not a member of. That is a meeting failing the "every member exactly once" rule this section states, held silently: coverage counts recorded meetings rather than complete ones, so nothing surfaces it and every figure looks ordinary. *This said the month "reconciles wrongly (Section 20)". It does not: both bucket views are computed from the attendees, so a roster with holes reconciles exactly as a complete one does. What the holes break is the rule above, which is why nothing detects them.*
...
**The legal moves are exactly four**, and every other move is refused as an `INVARIANT_VIOLATION`: a first submission is `HELD` or `NOT_HELD`; `HELD` → `RESCHEDULED`; `RESCHEDULED` → `RESCHEDULED`, since a meeting may move twice; and `RESCHEDULED` → `NOT_HELD`. Moving a `NOT_HELD` meeting is refused, because `NOT_HELD` means the meeting did not take place and is not being made up, and rescheduling contradicts the fact just recorded. `RESCHEDULED` → `HELD` is not a transition and needs none — `RESCHEDULED` already means the meeting took place on another date, and Section 12 counts both in N.

**A status recorded in error is corrected with a reason** (ruling of 2026-09-22, decision 0273). `HELD` → `NOT_HELD` and `NOT_HELD` → `HELD` are corrections rather than moves: the meeting did not change, the first report was wrong. Each requires a `correction_reason`, and is otherwise made as a move is — the same route, `cell.correct_subtree`, the meeting's `version`, and the window or an Admin amendment. It writes a `cell_meeting_changes` row carrying the reason as its note, and a `cell_meeting.status_corrected` audit entry, so the first report stays legible. A correction to `NOT_HELD` closes every mark with nothing replacing it and carries a `NOT_HELD` reason; a correction to `HELD` carries the whole roster of the scheduled date, as a first submission does. A `RESCHEDULED` meeting has none of these: it already has its own move to `NOT_HELD`.
...
**A leader lists the Cells of their scope** (ruling of 2026-09-09). `GET /api/v1/cells` returns the Cells whose leader falls within the actor's pastoral scope **at the instant Section 7 gives a collection naming a period** (ruling of 2026-09-10), guarded by `cell.view_subtree` and paginated by cursor (Section 22); `?led_by=me` narrows it to the Cells the actor personally leads. **`?state=CLOSED` is a view of its own** (ruling of 2026-09-21): the default is `ACTIVE`, because every count of Cells means active Cells unless it says otherwise, and the two are never mixed: a closed Cell offers a restart rather than a meeting, and its coverage denominator stops at its closure where a running Cell’s is the month’s whole schedule (Section 12). A closed Cell is in scope through its **last** leader, which is what Section 7 resolves a closed Cell through, and each row carries the day it closed, the reason, the Cell that has since resumed it where one has, and `may_restart` — whether this actor may ask for its restart (Section 10), answered by the server rather than derived by a client. It is the scope half of what the request checks, with the Cell being neither closed as created in error nor already restarted; a request already pending for that leader is not consulted, so a second one is refused by the request route. Section 19 wants both readings — its Cell-leader dashboard leads with "the user's own Cells" and its upline dashboard with "within their scope" — so the narrower one is a **filter over one authorized set** rather than a second route or a second rule. A Cell the actor leads is inside the actor's own subtree by definition, so the filter grants nothing and only removes rows the caller may already see.
...
- Cells with meetings still awaiting a record (Section 13), **including a closed Cell, for meetings whose month is still open**. Every other count of Cells means active Cells (Section 10); this one does not, because Section 13 gives the leader until the 7th whether or not the Cell survived, and a meeting nobody can see is a meeting nobody records. Bounded by the window rather than by how recently the Cell closed, so the list never shows a meeting only Admin could act on. The closed Cells reach it through `?state=CLOSED` (ruling of 2026-09-21), the `ACTIVE`-only listing having no way to carry one. **What a page bound owes this list is open**: both halves are read one page at a time, so a Cell beyond that page is absent from it, which was true of the running half before the closed one joined it. **Each meeting appears for the leader it names**, matching the dashboard (Section 19) and Section 7's answer for who may file it — a Cell handed over before it closed has meetings belonging to each leader, and showing them all to the last one would name somebody who cannot write them
- People with no active Cell membership within the viewer's scope (Section 10), at `GET /api/v1/cells/people-without-a-cell` (ruling of 2026-09-10). It is guarded by `cell.view_subtree` **against the actor**, with the scope narrowed in the module that runs the query — the shape the Cells index above already uses, though **which module that should be is recorded as open** rather than settled here, and a Cell capability rather than a People one because the act that resolves an entry is adding somebody to a Cell. It names no period and asks about now. **A person who leads an `ACTIVE` Cell is excluded**: a leader holds no membership row, so the literal reading would place every Cell Leader on a list of people needing a Cell. An archived Person and a merged-away one are excluded on the same ground as the sibling list in Section 5 — no act resolves their entry. Ordered by name, never by how long somebody has been without a Cell
...
**The screen opens on four of these lists, one at a time** (ruling of 2026-09-25, decision 0290): Awaiting a record, Cells behind, Needs a new leader and Not in a Cell, chosen by four equal buttons each carrying its count, a list read a page at a time counting what it has read and saying there is more. Awaiting a record is split into Cell meetings and DCC. Each list is a table, a card per row on a narrower screen; the two lists of people show everybody, fifty at a time. The reader's requests follow the lists, and the month's figures are a row at the foot.
...
rather than that one metric. Section 17 already requires a report to say that the period is
open.

**The quoted gloss — "which for the current period means now" — does not come with it**
(ruling of 2026-09-07). Section 16 attaches it because Section 3 classes that metric as a
*current-state* one, and current state has no future: there is nothing at the end of an
unfinished month for a current-state figure to report, and Section 16's participation window
is itself rolling and ends at the report's date. Classification and monthly attendance are
period-based on Section 3's own terms, and a calendar month's end is a fixed instant whether
or not it has arrived. The instant crosses Section 3's line, as the aside below says this
ruling intends; the gloss belongs to the metric it was borrowed from and stays there.

**So an open period resolves at the same instant a closed one does.** This section said
otherwise, three lines above the sentence it contradicted, for as long as both stood.

**The instant is the last millisecond of the period's final day** (ruling of 2026-09-05),
**open or closed** (ruling of 2026-09-07), which is what `endOfManilaDay` already computes
for Section 13's closure boundary. Not the
...
**A person whose pastoral leader holds no open pastoral assignment appears on an attention list**, shown to the upline who can act, on Section 15's terms — filtered, never ranked. *This read "whose pastoral leader is **archived**" until 2026-09-10. Archival is the commonest cause and was never the rule: the reconstruction above fires on a leader holding no assignment, which Section 5 gives three legitimate causes of, so the narrower wording surfaced one and hid two. Section 5 carries the list, its one exclusion, and the route.* **Who can actually see it is narrower than "the upline who can act" and is recorded as open**: the break that puts somebody on this list also removes them from every ancestor's subtree walk, so a subtree-scoped actor whose only path to the person runs **through the departed leader** is answered an empty list — which is every actor above the break, and so is exactly the upline this sentence names. It is not everybody: `subtreeOf` seeds at the actor, so the departed leader and the person themselves both see the row, and a `NETWORK` grant sees the rows of **its own Network**, because it enumerates by Network membership rather than by walking the tree — so a gap in the other Network stays invisible to it. The graph that would reach an upline is the placement graph above, which the ruling of 2026-09-06 forbids from authorizing anything. Required rather than advisable: a report that quietly reconstructs the chain removes the pressure to reassign anybody, so the transient state it accommodates would stop being transient.
...
This narrows *which people* the person key runs over. It adds no fourth attribution key: a Network-scoped figure is still attributed by the person, and a Cell-scoped one still by the meeting's responsible leader.

**For coverage it narrows the party coverage attributes by, and at coverage's own instant** (ruling of 2026-09-09). A `NETWORK`-scoped DCC coverage denominator is the leaders holding a pastoral edge at the event date whose `network_assignments` row names that Network **at that same instant**, and the numerator is those of them carrying a record. The narrowing is applied per event, because everything else about an obligation is resolved at the event — who was a responsible leader, and which subtree they sat in — and resolving the Network at the period's end instead would attribute a Sunday's obligation to whichever Network the leader finished the month in. Section 4 effective-dates membership so that "every Network-scoped report for a closed period depends on that answer", which is the same argument one key over. *This is stated because the paragraph above enumerates the person key and the responsible-leader key and omits coverage — the identical omission that made the ruling of 2026-09-08 necessary, one paragraph down. **What a `NETWORK`-scoped Cell figure narrows is a different question and stays open**: the Cell domain does not use the person key, so there is no stated narrowing to carry across, and `GET /api/v1/reports/cells/monthly` refuses that scope.* This claims no Men's + Women's identity for coverage; the one claimed above is for the unique-people total and rests on a property nothing enforces, which a coverage figure partitioned this way would inherit rather than replace.
...
**A report is computed inside one transaction, opened `READ ONLY` and at `REPEATABLE READ`** (ruling of 2026-09-05), and that is what makes the sentence above a property of the read rather than an accident of timing. Section 24 runs the application at `READ COMMITTED`, where every statement takes its own snapshot even inside one transaction, so two figures read separately can cover two populations and the identity fails with nothing raised. The first DCC report held the identity by taking every figure in **one statement**, which is sound at any isolation level and does not reach across modules: Section 2 requires the owning module to compute, so a leader-scoped report reads the tree from `hierarchy` and the figures from `attendance`, and coverage adds a third. Each module's own read stays one statement; the transaction is what carries the identity **between** them, and `REPEATABLE READ` is the half of it that does the work.

**This is a per-transaction override and never a change to `default_transaction_isolation`.** Section 24 records three mechanisms that depend on `READ COMMITTED` — the Section 5 person lock, the Section 7 grant-limit triggers, and the Section 6 bootstrap — each of which locks and *then* decides, and each of which would decide on a pre-lock snapshot if the default moved. A reporting transaction takes **no row or advisory lock** — the kind all three of those use — writes nothing, and authorizes nothing, so the two levels coexist. Nothing about the default changes. Section 24 states both of those clauses exactly, including what would falsify the second; this sentence defers to it rather than restating it, because the same claim written twice is the shape that leaves one copy uncorrected.

A stored figure for a closed month is computed once (below), so this governs the open month and any recomputation.
...
- Attendance amended after its month closed, with reason, in either domain. **It is written whoever the actor is**, unlike the on-behalf pair: an Admin amending their own Cell's meeting has still reached past a closed window. Without it an amendment that *records* a meeting nobody had reported would leave the month rewritten with nothing in the log, because a first submission writes no entry of its own. It is owed by an amending **correction** as much as by an amending first submission — a closed month usually already holds records, so the correction is the ordinary case rather than the exotic one
  - **Its granularity follows the domain's unit, and the two differ.** A Cell amendment is one entry targeting the Cell, which the bullet above already settles for every Cell attendance entry. A DCC amendment is one entry **per person written**, each targeting that person: a submission may name people belonging to many different leaders, and a single entry resolves through one target and would be invisible to every other leader whose people's figures moved. Section 14 makes the meeting the unit in one domain and the person in the other, and this follows the same seam. *A first implementation wrote one DCC entry targeting the amender's own Person, which put it in the amender's upline scope and out of reach of exactly the leaders it concerns.*
  - **Per person *written*, not per person named.** A line whose value already equals the stored one writes nothing and is not an amendment (Section 9), and this list asks for one entry per action performed — so an unchanged line earns no entry, and a resubmitted identical body earns none at all. The same holds of a Cell correction that corrects nothing. *Both wrote one anyway until 2026-09-04, so resubmitting told every named person's leader that a frozen figure had moved, once per resubmission.*
...
GET  /api/v1/cells                       the Cells of the actor's scope; ?led_by=me narrows, ?q= searches, ?state=CLOSED is the closed view (Section 15)
...
Refused rather than stripped or substituted: no section of this specification gives either character a meaning, so there is nothing for a domain layer to decide and nothing a leader meant by it. A refusal at the edge names the field, which is what a client needs in order to fix it; accepting the substitution stores text nobody typed. **An element of a typed array must be an object before it is validated as one.** A nested validator recurses into an array-valued element, finds no declared property on it and passes, so a malformed element reaches the domain layer — where it is refused for breaking a rule the request never broke, or reaches the database and answers `INTERNAL_ERROR`. Both were live: the Cell roster took the first and the DCC checklist the second, from the identical body.

**A refusal names where it is, and says what is wrong** (ruling of 2026-09-22, decision 0275). Each entry of `details.fields` carries `field`, the top-level property as it always has, `path`, which reaches inside it — `records[1].correction_reason`, `amendment.reason` — and `problems`, that member's messages. `path` and a filled `problems` are additions, so a client reading `field` alone is unaffected. A screen that sent a roster names the person on the refused line.
...
**A Growth record carries no version at all**, and its conflict names rows in place of versions (Section 28, decision 0282).

Those are the two **outcomes**, and the count is over outcomes rather than over everything a lost race may answer. Three things stand in front of them as guards on the way in, and none is a third outcome:

- A uniqueness violation on any *other* index is not a lost race at all and keeps failing loudly — the handler narrows on the index by name, because letting one surface on its own would answer `INTERNAL_ERROR` on an ordinary race, which is what naming these cases is for.
- **On a Cell meeting**, a submission whose **status** differs from the committed one is decided before the comparison, because a `NOT_HELD` body carries no attendance: the comparison would find nothing to disagree with and answer `RESOURCE_BUSY`, whose meaning is that the identical body resubmitted succeeds. It would not. Where the change is a transition Section 13 does **not** permit, the loser is refused exactly as it is refused when nothing raced. Where it is one of the four Section 13 does permit, the loser is a `VERSION_CONFLICT`, built only after the same amendment-capability check as the disagreeing branch below, and never `RESOURCE_BUSY` — a transition has no unchanged-submission path, so its version check is unconditional and the winner has already moved the version past what the body carries. DCC has no status and no counterpart to this guard. *This said "a status change is a separate operation", quoting a Section 13 rule that decision 0195 replaced with four legal transitions; the sentence outlived the rule by one branch, and the code answering with its wording outlived it too.*
- **On a Cell meeting**, the amendment capability is required of the disagreeing branch before its conflict is built, because that conflict carries the stored figures and the submitter's name. *The ground given here until 2026-09-09 was "which no read route hands an actor holding only the recording capability", and the roster ruling of that date made it false: `GET /api/v1/cells/{id}/meetings/{meeting_id}/roster` now carries each member's mark and already returned the submitting **account**, under `cell.take_attendance` alone. *The account rather than the name: the conflict resolves it to a person's name and the roster returns `submitted_by` as an identifier. The marks half is what falsifies the old ground; the submitter half is by identifier.* The gate stands on the rule rather than on being the only door — a refusal must not answer what the capability withholds, whatever another route publishes.* It is *not* required of the agreeing branch, which writes nothing and owes no amendment capability under the rule in Section 7 — asking for it there refuses a write this specification permits, and stores the refusal against the idempotency key.

**The second and third are stated of the Cell path only, and the DCC path does not implement the third.** `DccAttendanceService` builds its lost-race conflict without asking for `dcc.correct_subtree`, so an actor holding `dcc.take_attendance` alone is answered `409` with the stored value and the recording account's name where the identical body sent sequentially answers `403`. The disclosure argument is the same one that settled the Cell path. *It was stated here as "the DCC roster publishes neither the actor's identity", which is half-edited and, on the reading its neighbours give it, false: that roster returns each person's `present`, `version` and `recorded_at` (above), so the stored figure is published to that actor for everyone on their own checklist, and only the recording account's **name** is withheld outright. *Not to exactly the same set: the roster walks the checklist while the submission reaches everyone the actor holds `dcc.take_attendance` over, which is the difference stated above and the reason the Cell ground was ruled not to transfer.* What makes the asymmetry look like a defect is the ordering rule itself — a refusal must not answer what the capability withholds — and not a claim about which route publishes what.* It is **recorded as open in `CLAUDE.md` rather than asserted here**, because the rule that would settle it is the one that section already records as unsettled, and because a Cell argument asserted over DCC by a fix batch is the mistake decisions 0193 and 0194 exist to record. A version of this bullet made exactly that mistake a third time.

*Each of the three was a live defect first and a sentence afterwards, on the Cell first-submission race in 2026-09-03: the uniqueness violation surfaced as a 500, the status disagreement answered `RESOURCE_BUSY` with a retry that could never succeed, and the amendment capability was first absent, then demanded of everyone, then stated over a domain that does not implement it. Each was introduced by the fix for the one before.*

It is said separately because this section states counts in order that they can be checked, and a count of outcomes that quietly included guards would not be checkable at all.
...
**Two rendering engines are supported, and they are what the conformance claim below is tested against: Blink and WebKit — each for a different thing** (ruling of 2026-09-10). **Blink** carries the automated conformance check: axe-core over every route, in both themes, a violation failing the build. **WebKit** carries the layout check, at the two viewport widths that bind — the narrowest, where overflow is hardest, and the widest at which anything changes. Axe is not run twice: its rules are computed from the accessibility tree and the resolved styles, which is not where the engines differ, while layout is; and a second full engine roughly doubles a job that is worthless if it becomes slow enough to skip. Blink covers Chrome, Edge and every Chromium browser; WebKit covers Safari, and with it *every* browser on iOS, since iOS permits no other engine — so Chrome on an iPhone is WebKit and a Chromium-only check says nothing about any iPhone. Gecko is not supported: no phone or tablet in use here runs it, and no leader has asked for Firefox. Supporting it later is an ordinary decision and not an amendment to this section; claiming iOS works without testing WebKit is what this rule forbids.
...
**One kind of transaction deliberately runs at another level, and it is named here so that the default above stays the only global answer.** A report is computed inside one `READ ONLY` transaction at `REPEATABLE READ` (Section 20, ruling of 2026-09-05), because Section 20's two reconciliation identities require its several reads to cover one population and `READ COMMITTED` gives a fresh snapshot per statement. It is compatible with everything above because it shares none of their shape: the three dependencies named here are all *lock, then decide*, and a reporting transaction takes **no row or advisory lock**, writes nothing, and decides no authorization. **"No lock" would be false and the narrower claim is the one that carries the argument** — a `READ ONLY REPEATABLE READ` transaction holds an `AccessShareLock` on every relation it reads for its whole duration, which conflicts with nothing above and does conflict with DDL, so a long report and a migration must not be run against each other. **The third clause was a fact about the system as it stands and is now a rule** (ruling of 2026-09-06): a report's authorization is decided **before** its transaction opens, on the pooled connection where every other guard in this application decides, so nothing authorizes inside one of these transactions by design rather than by absence. Measuring an actor's reach against the same snapshot a report is computed from would falsify this clause, and is refused for that reason among others — Section 7 carries the rule and the rest of the reasoning. **The first reporting route owes a test that the guard decides before the transaction opens**, stated here rather than beside the route because this clause is what the test protects: a rule with nothing that can fail on it is a wish, which is the standard the mechanism two paragraphs up is already held to. *This paragraph rested the clause on there being no reporting route. That observation is replaced by the rule rather than by a route: none exists yet, and the clause is to hold when one does. A fact a reader would go on trusting after it stopped being true is exactly what this replaces.* **A report is the only exception, and adding a second is an amendment here** rather than a choice made in the module that wants it.
...
- **Open a cell** — they hold the earliest leadership of a Cell that an approved `NEW_CELL` request names (Section 10), or of a Cell recorded during initial encoding (Sections 2 and 10), dated the day that leadership began (ruling of 2026-09-18). A handover is not an opening. A Cell recorded at setup already existed, so the records cannot tell whether its first recorded leader opened it or took it over; it counts as opened, and the cost is stated rather than hidden: somebody who took a Cell over before the church was encoded reads as having opened it.
...
**A Cell closed `CREATED_IN_ERROR` does not count toward Open a cell** (ruling of 2026-09-24, decision 0283). That closure says the Cell should never have existed (Section 10), so it is the correction of the record the goal rests on, and it unmakes a goal reached through that Cell as any correction does (*Reached once is reached*, below). A Cell closed for any other reason still counts: it existed and was opened. **Nor does leading such a Cell count toward Raise 12 leaders for any pastoral leader its leader had while leading it** (decision 0286), so the two goals treat it alike.
...
**A correction unmakes a reached goal**, and it is not an exception to this rule but what a correction means: withdrawing a confirmation, or a Cell leadership, a pastoral assignment or a SUYNL lesson a derived goal was resting on, says the record was wrong rather than that the person has fallen back (Section 28). So does closing a Cell `CREATED_IN_ERROR` (decisions 0283 and 0286). Whether anything else does is not claimed here. The ground for the rule itself is what the ladder means: a milestone reached is not a level somebody falls out of. Section 3's reproducibility guarantee is not a second ground and is not claimed as one, since Section 3 and Section 16 already provide for a current-state figure that is reproducible for a closed month.

### What a leader confirms

A leader confirms one thing, and it is a dated record naming them: **a goal reached before the church was encoded.** The import fabricates no history (Section 2), so twelve disciples held in 2024 are in no table, and neither are the lessons of somebody discipled in 2019. The leader states it, with the date, and **that date is not the backdating of Section 3** (Section 28, ruling of 2026-09-16) while no report counts these goals by period. Win 3 reaches this path exactly as the other three do, and until SUYNL has been recorded for a while it is the only path there is for most people.

**Nothing is inferred from the rung below.** A person who opened a Cell is not thereby recorded as having won three: no rule of this church requires three before a Cell is opened, so the inference would be only as true as a rule nobody has written. Win 3 is read from SUYNL lessons instead.

**The ladder is displayed in order and never gated.** An out-of-order state is reachable three ways: a Cell opened by somebody against none of whose disciples three lessons are recorded; somebody who took a Cell over reaching rungs three and four while rung two stays permanently false; and a pre-encoding confirmation of a later rung while rung one stands unreached. A refusal could reach only what a leader states, which is the one confirmation above, a goal reached before encoding.

### Who sees Conquest, and who may confirm

Three capabilities, in the shape Section 9 uses for DCC attendance:

- `conquest.view_subtree` — read the goals of anyone in the actor's pastoral subtree. A read capability (Section 7), resolving as of now by the ruling of 2026-09-23, which Section 28 states for all three of this domain's read capabilities at once.
...
**The two Network roots are reached by neither confirming capability**, being nobody's direct disciple and nobody's downline. They are confirmed for by any actor whose `conquest.confirm` grant is Whole Church, other than the root themselves, as Section 9 puts a root on a checklist — **nobody files or corrects a confirmation about themselves** (ruling of 2026-09-23, decision 0280, which Section 28 states for all three tabs) — and their rows carry no confirming leader: `confirmed_by` is null for a Network root and for nobody else, as `dcc_attendance.responsible_leader_id` is.
...
- reached_on           date, required: when the leader states the goal was reached
...
**A row states only that a goal was reached.** Nothing records that one was not; a person with no row for a goal has not had it confirmed.
...
**Only SUYNL is recorded lesson by lesson, and that is a recording decision rather than a claim about the materials.** Life Class also runs to nine lessons and is recorded here as a single graduation. An enrolment application planned elsewhere will own enrolment and lesson progress for the schools; recording their lessons here as well would give the church two records of one fact, free to disagree, with nothing able to keep them in step.
...
**The next season is shown as four steps**: from ten weeks before the weekend, the LC Party, Life Class lessons 1 to 4 from the week after it, and the Encounter. The ten weeks are a reminder and refuse nothing.
...
**The separation is of the records and not of everything derivable from them.** A holder of `conquest.view_subtree` alone reads Win 3, and so learns that three of a person's disciples each carry three lessons — a fact about rows they hold no `suynl.view_subtree` over; and a holder of `suynl.view_subtree` alone learns, from who SUYNL's readiness table leaves out, that a person holds a current Encounter or Life Class graduation. Nothing leaves the reach of the grant that returned it, the derived fact being about people inside the Conquest grant's own scope, and it is stated here because the sentence above would otherwise read as a completeness it does not have.

**The two Network roots are reached by neither confirming capability**, being nobody's direct disciple and nobody's downline. They are filed for by any actor whose `suynl.confirm` or `training.confirm` grant is Whole Church, other than the root themselves, as Section 9 puts a root on a checklist. **Nobody files or corrects a SUYNL lesson, a graduation or a Conquest confirmation about themselves** (ruling of 2026-09-23, decision 0280): a write naming the actor's own Person is refused whatever their grant, and their own row is shown without boxes. Their rows carry no confirming leader: `confirmed_by` is null for a Network root and for nobody else.
...
**The read capabilities of this section and of Section 27 resolve as of now** (ruling of 2026-09-23) — `suynl.view_subtree` and `training.view_subtree` here, `conquest.view_subtree` there — taking Section 7's default rather than joining its closed list of three that resolve as of the period being viewed. These screens name no period and the counts are stated as of now, so there is no period to resolve at.

**What it forecloses, rather than what it changes.** Nothing behaves differently today: an undated viewing request asks about now under either reading (Section 7), and nothing here names a period. What the ruling settles is that a route which later does name one cannot inherit a dated resolution from this class — it would need Section 7's closed list amended, and its own ruling.
...
**A save made against a view that has since changed is refused** (ruling of 2026-09-24, decision 0282). Each change names the row it was made against, or none; where that row is no longer current, nothing is saved and the first such line is named, as a `VERSION_CONFLICT` carrying `submitted_row` and `current_row` in the places of Section 22's two versions. A change that already agrees with what is stored writes nothing and conflicts with nothing.
...
**Those counts have a stated population.** Each is `COUNT(DISTINCT person_id)` over exactly the people the tab lists — every current Person the actor's `suynl.view_subtree` or `training.view_subtree` grant reaches **at the scope that grant carries**, resolved as things stand **now**, the screen naming no period — of those who have reached that step. **SUYNL has three cards — Not started, In progress (one to nine lessons) and Graduated — which add up to everyone listed; Training has one per graduation, which overlap, and one for people with none yet** (ruling of 2026-09-24, decision 0281). **A count filters this tab's own list to the people behind it**, which Section 27's four also do. **The SUYNL and Training lists open on the people still to finish** — short of ten lessons, or of all five graduations — with a line saying how many are left out and a way to show everyone; a card, a search or "Only my disciples" shows whoever it names, and the counts still cover everyone listed (ruling of 2026-09-24, decision 0287). Nothing is ranked or compared across scopes (Sections 13, 17 and 19), and nothing is colour-graded. The one attribution to a leader is the readiness table's rows, each a branch as it stands now.
