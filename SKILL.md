...
- **On a Cell meeting**, a submission whose **status** differs from the committed one is refused before the comparison, exactly as it is refused when nothing raced (Section 13: a status change is a separate operation). It cannot take either outcome — a `NOT_HELD` body carries no attendance, so the comparison would find nothing to disagree with and answer `RESOURCE_BUSY`, whose meaning is that the identical body resubmitted succeeds. It would not. DCC has no status and no counterpart to this guard.
