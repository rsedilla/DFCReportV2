...
**And a third option exists that nobody has costed**, which is why this is not settled here: Section 22 says a meeting's roster is guarded by "`cell.take_attendance` for a submission and `cell.correct_subtree` for a correction", while the route declares the first alone. Carrying `record` only for an actor holding the second is a reading this section can support and this specification has not chosen. Which capability may read a Cell meeting's recorded marks is **recorded as open in `CLAUDE.md`**: three statements give three answers, and the field exists to serve the correction case.
...
- **On a Cell meeting**, the amendment capability is required of the disagreeing branch before its conflict is built, because that conflict carries the stored figures and the submitter's name, which no read route hands an actor holding only the recording capability. It is *not* required of the agreeing branch, which writes nothing and owes no amendment capability under the rule in Section 7 — asking for it there refuses a write this specification permits, and stores the refusal against the idempotency key.
