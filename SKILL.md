...
The rule is about *when* the instant is read and not about which clock reads it — `clock_timestamp()` after the lock satisfies it as `new Date()` after the lock does. It reaches the operations that compute a floor, which is what makes the ordering matter; an operation with no floor to breach is unaffected, and the tree import deliberately stamps one instant before its transaction so that every row of the tree shares an effective date (Section 2).
