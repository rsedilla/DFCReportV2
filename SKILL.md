...
The warning does not claim the ordinary rows are freely correctable, because they are not quite: nothing in this system closes a pastoral assignment without opening a replacement, so a reassignment corrects the **leader** on an edge and cannot remove the subject from the tree. A `USE_EXISTING` naming the wrong Member ID on an ordinary row leaves that Person placed, counted in a subtree that does not contain them, with only their leader correctable. The root case differs in degree rather than in kind — the seat is unique and the Network is frozen — and the report says which rows carry that degree.
