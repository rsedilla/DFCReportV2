...
- a **report scope selector** is itself the target: a request for a scope the actor does not hold is `SCOPE_DENIED`, never silently narrowed to what they do hold
  ...
  - **The reach is measured against the placement graph, which is the graph the figure is computed from** (ruling of 2026-09-05). Section 20 places a person by their open assignment at the period's end, by the last one they held within it, or up the chain past a leader who had already left — a graph strictly wider than the assignments in force at any one instant. Measuring reach against the narrower one reproduces, through the graph, the failure the instant rule above was written to prevent: a leader would be shown a total containing a person and refused the breakdown that explains them. It is correct rather than merely permissive, on this section's own criterion — a person unassigned at the period's end was discipled within that period by the leader the fallback names
  - **One edge definition serves both directions.** Resolving reach walks *up* from the target while a report's figures walk *down* from the actor, and both must be built from the same edges rather than derived twice. Where the two disagree the failure is asymmetric and neither half is benign: an upward walk wider than the downward one authorizes a leader for people no figure contains, and a narrower one refuses a leader the drill-down into their own total
...
**Where the graph is not functional, the figure refuses the totals that are corrupted, and only those** (ruling of 2026-09-05). The whole-graph argument above holds only while each person has one out-edge, and two `pastoral_assignments` rows in force at one instant break it — a cycle member with a second, grounded edge grounds the whole cycle and is not detected, and a person reached by two paths is returned twice, which Principle 10 forbids of any total of people. So a walk whose own result names a person more than once refuses, on the same terms as a cycle. **The two refusals are scoped differently, deliberately.** A cycle is invisible from a total that should contain it, so detection must reach past the walk; a duplicate corrupts only a total that contains it, and a leader whose subtree holds no duplicated person keeps a figure that is correct — refusing that one would punish a leader for another branch's state, which is the ground this section's fallback already gives for not refusing a whole report. Scoping loses nothing: a walk reaching anyone beneath a cycle passes through the cycle to get there. **Choosing between the two rows is refused rather than automated**, because it decides which of two leaders the person belongs to for that period, permanently, and nothing here decides that. Whether two rows may overlap at all is a Section 5 question recorded as open in `CLAUDE.md`, with a database constraint as its remedy.
...
**One kind of transaction deliberately runs at another level, and it is named here so that the default above stays the only global answer.** A report is computed inside one `READ ONLY` transaction at `REPEATABLE READ` (Section 20, ruling of 2026-09-05), because Section 20's two reconciliation identities require its several reads to cover one population and `READ COMMITTED` gives a fresh snapshot per statement. It is compatible with everything above because it shares none of the shape: the three dependencies named here are all *lock, then decide*, and a reporting transaction takes no lock, writes nothing and decides no authorization. **A report is the only exception, and adding a second is an amendment here** rather than a choice made in the module that wants it.
