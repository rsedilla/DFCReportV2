...
**A facilitator must be a Person who exists, and that is refused rather than left to a foreign key** (ruling of 2026-09-04). It is the only field on this route carrying an identifier a caller chooses and nothing else resolves, so a well-formed body naming an absent Person reached the constraint and answered `INTERNAL_ERROR` — Section 22's named failure mode. *What the field may name **beyond** existing is deliberately not settled here: any Person, a member of that Cell, or someone in the leader's subtree are three readings that refuse different bodies, and existing is the part all three share. It is recorded as open in `CLAUDE.md`.*
