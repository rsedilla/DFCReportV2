...
- Internal primary key: UUID recommended
...
Therefore a calendar month has exactly 4 or 5 applicable DCC events, determined by the actual number of Sundays in that month — never an unexplained arbitrary limit.
...
Cell attendance does not automatically create or end Cell membership. Membership changes only through an explicit, authorized workflow (e.g. a leader or admin adding or removing a member). Attendance at a Cell other than a person's assigned Cell, if ever supported, must not automatically transfer their assignment or alter their monthly denominator. The detailed workflow/permissions for managing membership are defined during Cells module implementation planning.
