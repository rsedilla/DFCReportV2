...
**The bound covers the whole transaction, not only the advisory acquisition** — where one is set at all. It is set when the advisory locks are taken, and stays in force for the row locks the caller's own writes take afterwards; an operation that takes **no** advisory lock, because it has nobody to lock, sets no bound and must set one itself before it takes a row lock. That case is real rather than theoretical: a Cell closure with no members to disperse is exactly it — including the idempotency key's. That is deliberate: those waits are unbounded otherwise, and an unbounded wait anywhere inside a transaction holding a pooled connection is the same hazard. It follows that an elapsed wait must answer `RESOURCE_BUSY` **wherever it is raised**, including at a call site that knows nothing about locks; classified as an unexpected failure it would be a 500 for ordinary contention.

**Ordering advisory locks against row locks is not settled, and no operation in this system yet needs it.** Every path that takes an advisory lock today takes only advisory locks, so the ascending-lock-key rule above is sufficient for all of them. Section 10's Cell closure will be the first operation needing both classes at once, and it establishes the ordering — against the database rather than in prose.
...
It cannot be settled here because it depends on a question this section leaves open: whether the rule that no row of a closed Cell may end after the Cell did reaches category and schedule rows, or only leadership and membership rows, which are the two the database constrains today. The closure endpoint settles both together, against the schema rather than against prose, and states the floor then.
...
**The floor says how far back a closure may be dated; `records.backdate_effective_date` says who may date it back at all.** Any effective date earlier than the current day requires that capability (Section 7), which is Admin's. The closer may always date a closure today, so nothing is blocked — what they give up is an accurate scheduled-meeting count for the days between the Cell's last meeting and the closure being recorded.
...
Three orderings were written for this and each was refuted, the last by reproducing a deadlock against the database. The reason they failed is worth stating so the fourth is not attempted the same way: a deferred constraint trigger takes row locks at commit, in the order rows were written, which no rule reaches after the fact; and a closure cannot know which people to lock until it has read its member list, so any ordering that puts the Cell first must then reach back for people, and any ordering that puts people first reads a list that can be stale.
