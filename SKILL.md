...
**One thing is exempt, and it is named rather than left to interpretation: a read joined onto a query rooted in a table the reading module owns.** Two modules qualify, in three places. `hierarchy` joins `persons` in two places — `openDisciplesOf`, to name a leader's open direct disciples, and `directLeaderNameOf`, to name one leader. Both start from `pastoral_assignments`, which `people` does not own and cannot query, so the join cannot move to the owning module; returning identifiers for the caller to resolve moves it rather than removing it, and for `openDisciplesOf` turns one query into one per row. **`people` anti-joins `cell_memberships`, `cell_leaderships` and `cells` in one place** — `withoutACell`, the attention list Section 15 requires, which asks which people hold no Cell and so is rooted in `persons` (ruling of 2026-09-10). It cannot move to `cells`, which owns no table containing the people who hold none. Nothing else qualifies today, and adding to this is an amendment rather than a decision taken in a module — widened once, by a ruling with all three of its legs, which is this list staying closed rather than opening.
