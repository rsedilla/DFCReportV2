...
**A request refused because of such a row answers `CAPABILITY_DENIED`, not `SCOPE_DENIED`** (Section 22). This is the opposite of the rule below for a capability held at a scope that covers nothing, and the two differ on the thing those codes distinguish: whether the capability is held at all. A grant issued at a narrower scope than this section permits still *names* the capability, so the account holds it and only the scope is unusable. A refused `SENIOR_PASTOR` row names nothing, so the account holds none of the role's capabilities at any scope, and `SCOPE_DENIED` would send an administrator to widen a scope that does not exist.
