...
**A lost race does not always produce a conflict, and the third outcome is `RESOURCE_BUSY`.** Where the winner recorded the value the loser was carrying, the loser's line is unchanged against the committed state — so it takes no part in the version check and there is nothing to choose between. The identical body resubmitted then succeeds, writing nothing, which is what `RESOURCE_BUSY` means and what the third condition in the table above names. Answering a conflict there would present two identical values; letting the uniqueness violation surface would answer `INTERNAL_ERROR` on an ordinary race, which is what naming these cases is for.
