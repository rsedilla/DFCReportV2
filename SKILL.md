...
- People with no active Cell membership within the viewer's scope (Section 10), at `GET /api/v1/cells/people-without-a-cell` (ruling of 2026-09-10). It is guarded by `cell.view_subtree` **against the actor**, with the scope narrowed in the owning module — the shape the Cells index above already uses, and a Cell capability rather than a People one because the act that resolves an entry is adding somebody to a Cell. It names no period and asks about now. **A person who leads an `ACTIVE` Cell is excluded**: a leader holds no membership row, so the literal reading would place every Cell Leader on a list of people needing a Cell. An archived Person and a merged-away one are excluded on the same ground as the sibling list in Section 5 — no act resolves their entry. Ordered by name, never by how long somebody has been without a Cell
