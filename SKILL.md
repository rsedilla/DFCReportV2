...
- a **report scope selector** is itself the target: a request for a scope the actor does not hold is `SCOPE_DENIED`, never silently narrowed to what they do hold
  - **A `NETWORK` selector is covered by a Whole Church grant, or by a `NETWORK` grant naming that Network, and by no subtree grant at all** (ruling of 2026-09-07). That last clause is not a separate rule: Section 20 makes a Network's population its *membership* rather than any subtree, and its residual — somebody no leader discipled, who still holds a Network row — is the proof that the two differ. So a holder of `OWN_SUBTREE` or `SUBTREE_EXCL_SELF` never holds a whole Network, at a Network root included, whose subtree still excludes that residual. Nobody is affected in practice: both Senior Pastors hold Whole Church (Section 4), which is what makes the two Network selectors Section 17 offers them work
...
Where a person held no open assignment at **any** instant of the period, and none before it either, they appear in the Whole Church total alone. No leader discipled them in that period, and attributing them to one would invent a pastoral relationship the tree never held.
