...
- a **report scope selector** is itself the target: a request for a scope the actor does not hold is `SCOPE_DENIED`, never silently narrowed to what they do hold
  ...
  - **A grant at `NETWORK` scope covers no record here** (ruling of 2026-09-07). Datedness is fixed to the capability, and no dated Network resolution exists: `currentNetwork` answers about now, so a report for a closed month authorized that way would be measured against whatever Network the leader holds today, which Section 4 permits to differ. **The refusal does not turn on which period was asked for, nor on which selector** — what covers no record is the grant. A leader-scoped selector has no dated resolution; a Whole Church one is never covered by a narrower grant. Both reasons are in the message, because neither is "not over this record": no target works for such a grant, which is the lie this section draws the distinction to avoid. The request is refused, and refused where the grant is read rather than where the tree is walked, so the message names the grant — no target works for such a grant, and "not over this record" would be the lie this section already refuses to tell for a capability granted too narrowly. **What a Network grant of a dated capability *ought* to mean is not settled by this**, and is recorded as open in `CLAUDE.md`: either this section states that such a grant resolves undated and why, or a dated route owes a `network_as_of` resolution. Refusing decides nothing about Networks; it declines to authorize where no rule exists.
