...
**One thing is exempt, and it is named rather than left to interpretation: a read joined onto a query rooted in a table the reading module owns.** `hierarchy` joins `persons` in two places — `openDisciplesOf`, to name a leader's open direct disciples, and `directLeaderNameOf`, to name one leader. Both start from `pastoral_assignments`, which `people` does not own and cannot query, so the join cannot move to the owning module; returning identifiers for the caller to resolve moves it rather than removing it, and for `openDisciplesOf` turns one query into one per row. **`people` anti-joins `cell_memberships`, `cell_leaderships` and `cells` in one place** — `withoutACell`, the attention list Section 15 requires, which asks which people hold no Cell and so is rooted in `persons` (ruling of 2026-09-10). It cannot move to `cells`, which owns no table containing the people who hold none. **The instances are not enumerated here** (ruling of 2026-09-11). `api/module-boundaries.json` holds every one, and `api/scripts/check-module-boundaries.mjs` derives them from the tree on every `npm run lint`, failing on a cross-module read the ledger does not name **and** on a ledger entry that matches no read. The three above are argued because their reasons are the argument for the shape; the list itself lives where something can fail on it. A total in prose is the part of a rule nothing checks, and this one was wrong three times — the third time in this very sentence, which claimed a closed list while `cells` joined `persons` in two places it did not name.
