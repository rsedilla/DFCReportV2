...
**A backdated reassignment is bounded by the same floor a backdated Network correction is** (Section 4), and by one rule of its own. It is not the same pair: Section 4's second bound is on the Network row, which a reassignment does not write.

- **Strictly later than the floor of Section 4** — the latest of the `started_at` of the person's current assignment and the `ended_at` of every already-closed assignment touching them, in either direction. The same floor, computed by the same code.
