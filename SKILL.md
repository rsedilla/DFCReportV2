...
*Stated over what the caller is told rather than over what the code reads, and that is the rule rather than a gloss on it.* Both implementations load a record and compute against it before deciding the on-behalf capability, and neither can avoid it: the amendment capability is decided **by** the contents, so they have to be in hand. What must not happen is a refusal, a success, or a status code that differs according to them. A version of this sentence said "before the record's contents are read", which no implementation obeyed and which was rescued by a paragraph below contradicting it — the fourth narrowing of one sentence in four commits, and the first attempted by gloss rather than by amendment. Ordered the other way the *success* answers what the refusal was withheld to protect, which is a defect this specification has now been given twice — and it is why `dcc.submit_on_behalf` is required of an unchanged line while `dcc.correct_subtree` is not.
...
- **The figures are mostly the actor's own to write.** Section 9 makes a person's attendance their direct pastoral leader's obligation, and a checklist is prefilled because most of its lines repeat what is already recorded (Section 9) — a leader who cannot see what is marked re-asks people already counted. *Mostly, not wholly: where a record exists and disagrees, changing it needs `dcc.correct_subtree`, so there is a figure such an actor may read and may not write. An earlier version of this sentence said "withholding it would protect a figure the same actor may write" without that qualification, which is true only of a record that does not yet exist.*

So the acceptance above rests on two different grounds in the two domains, and they are stated separately rather than merged. A Cell meeting's bit costs 2^N because the submission names every member at once and is answered as a whole. DCC is recorded per line with per-line outcomes, so no such bound exists — and none is needed, because the argument there is that the actor is the person recording. *This clause said "which none does today", of the whole system, and was false when written; a version that noticed left the consequence open and is now settled.*
