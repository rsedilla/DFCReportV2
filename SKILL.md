...
**A field inside a nested object is named by its container only** — the refusal says `records` or `amendment` rather than the member that broke, and carries no message. That is the error filter's existing shape rather than this rule's, it is the same for every nested refusal, and it is recorded as open in `CLAUDE.md`.
